...
final class TerminalTests: XCTestCase {
    func testExtendedEnterAndPreservationAtDecoderBoundary() throws {
        for sequence in [
            "\u{1b}[27;2;13~", "\u{1b}[13;2u", "\u{1b}[27;3;13~", "\u{1b}[13;3u", "\u{1b}\r", "\u{1b}\n", "\u{0e}",
        ] {
        ...
        for sequence in ["\u{1b}", "\u{1b}[27;2;", "\u{1b}[Z"] {
            ...
            XCTAssertEqual(
                try TerminalKeys(read: { _ in bytes.isEmpty ? -2 : bytes.removeFirst() }).next(),
                .unrecognized(Array(sequence.utf8)))
        }
    }
    ...
    func testHighlightingReusesGrammarsAndProtectsStringsComments() {
        ...
        XCTAssertEqual(
            regex.stringByReplacingMatches(
                in: rendered, range: NSRange(rendered.startIndex..., in: rendered), withTemplate: ""), assembly)
    }

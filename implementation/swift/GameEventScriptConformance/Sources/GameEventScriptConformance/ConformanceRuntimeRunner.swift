...
public enum ConformanceRuntimeRunner {
    ...
    public static func runCase(_ testCase: ConformanceCase, programs: [ConformanceRuntimeProgram]) -> ConformanceCaseResult {
        do {
            guard ["scriptApi", "loadError", "performance"].contains(testCase.kind) else { throw ConformanceExecutionError.invalidInput("Not a runtime scenario") }
            let ids = testCase.sources.reduce(into: [String]()) { if !$0.contains($1.programID) { $0.append($1.programID) } }
            guard ids == programs.map(\.id) else { throw ConformanceExecutionError.invalidInput("Missing, reordered, or extra Program group") }
            ...
            for _ in 0..<(testCase.metadata["hostCount"]?.integerValue ?? 1) {
                ...
                if testCase.kind == "loadError" {
                    ...
                    } catch let error as GameEventScriptDynamicLinkError { scenario.checkDiagnostic(testCase.expectation["error"] ?? .null, error.diagnostic, path: "/error") }
                } else {
                    try scenario.run()
                }
        ...
        } catch { return .init(testCase: testCase, status: "error", code: "conformance.runner.unhandledException", missingCapabilities: [], mismatches: [], technicalDetails: String(describing: error)) }
    }
}
...
final class RuntimeScenario {
    ...
    static func makeHost(_ test: ConformanceCase, observer: (any GameEventScriptRuntimeObserver)?, publishSink: (any GameEventScriptPublishSink)?) throws -> GameEventScriptHost {
        ...
        let setters: [String: WritableKeyPath<GameEventScriptRuntimeLimits, Int>] = [
            ...
            "maxCallDepth": \.maxCallDepth, "maxRandomScopeDepth": \.maxRandomScopeDepth, "maxRangeItems": \.maxRangeItems, "maxGeneratedCollectionItems": \.maxGeneratedCollectionItems, "maxDiceCount": \.maxDiceCount, "maxDiceSides": \.maxDiceSides,
        ]
        for entry in test.metadata["runtimeLimits"]?.objectValue ?? [] { if let key = setters[entry.key], let number = entry.value.integerValue { limits[keyPath: key] = number } }
        ...
        if let entropy = random?["entropy"]?.stringValue {
            ...
            for index in stride(from: 0, to: scalars.count, by: 2) {
                guard index + 1 < scalars.count, let byte = UInt8(String(decoding: scalars[index...index + 1], as: UTF8.self), radix: 16) else { throw ConformanceExecutionError.invalidInput("Invalid entropy bytes") }
                bytes.append(byte)
            }
            seed = try GameEventScriptRandomGenerator.seedFromEntropy(bytes)
        }
        let sequence = try (random?["sequence"]?.arrayValue ?? []).map { try ConformanceRuntimeValueCodec.number($0.numberValue ?? $0.stringValue!) }
    ...
    func configure() throws {
        ...
        for (index, definition) in test.metadata.values("nativeHandlers").enumerated() where definition["initiallySubscribed"]?.boolValue != false { _ = try subscribe(id(definition, index)) }
    }

    func run() throws {
        try configure()
        _ = try host.start()
        if host.isReady && test.expectation["initialization"]?["pump"]?.stringValue != "start" { _ = try host.runToCompletion() }
    ...
    func subscribe(_ identifier: String) throws -> Bool {
        ...
        for (index, definition) in test.metadata.values("nativeHandlers").enumerated() where id(definition, index) == identifier {
            ...
            if definition["messageName"]?.boolValue == true {
                subscriptions[identifier] = try host.subscribeMessageName(definition.text("message"), handler: handler, priority: definition["priority"]?.integerValue ?? 0)
            } else {
                subscriptions[identifier] = try host.subscribe(.init(name: definition.text("message"), parameters: definition.values("parameters").compactMap(\.stringValue)), handler: handler, priority: definition["priority"]?.integerValue ?? 0)
            }
            return true
        }
        return false
    ...
    func compare(_ expected: ConformanceData, path: String) throws {
        ...
        if case .object(let starts) = expected["programStarts"] {
            for entry in starts {
                ...
                if actual != expectedState.stringValue { mismatch(path + "/programStarts/" + id, expectedState.stringValue ?? "", actual) }
            }
        }
        ...
        if diagnostics.count != collector.diagnostics.count { mismatch(path + "/diagnostics/length", String(diagnostics.count), String(collector.diagnostics.count)) }
        for (index, pair) in zip(diagnostics, collector.diagnostics).enumerated() { checkDiagnostic(pair.0, pair.1, path: path + "/diagnostics/" + String(index)) }
        var offset = 0
        for limit in expected["runtimeLimits"]?.values("include") ?? [] {
            if let index = collector.limits.indices.dropFirst(offset).first(where: { matches(limit, collector.limits[$0]) }) { offset = index + 1 } else { mismatch(path + "/runtimeLimits/included", String(describing: limit), "missing") }
        }
        for limit in expected["runtimeLimits"]?.values("exclude") ?? [] where collector.limits.contains(where: { matches(limit, $0) }) { mismatch(path + "/runtimeLimits/excluded", "absent", String(describing: limit)) }
        if let trace = expected["trace"]?.arrayValue {
            if trace.count != collector.trace.count { mismatch(path + "/trace/length", String(trace.count), String(collector.trace.count)) }
            for (index, pair) in zip(trace, collector.trace).enumerated() { try compareEvent(pair.0, pair.1, path: path + "/trace/" + String(index)) }
        }
    }
    ...
    func channel(_ expected: [ConformanceData], _ actual: [GameEventScriptMessage], path: String) throws {
        ...
        for (index, pair) in zip(expected, actual).enumerated() {
            if try !ConformanceRuntimeValueCodec.messagesEqual(pair.0, pair.1, comparison: test.metadata["comparison"]) { mismatch(path + "/" + String(index), String(describing: pair.0), pair.1.description) }
        }
    }

    func checkDiagnostic(_ expected: ConformanceData, _ actual: GameEventScriptDiagnostic, path: String) {
        let fields: [String: String?] = [
            "phase": String(describing: actual.phase), "code": actual.code, "symbol": actual.symbol, "symbolKind": String(describing: actual.symbolKind), "programName": actual.programName, "handlerName": actual.handlerName,
        ...
        for (key, value) in [("line", location?.line), ("column", location?.column), ("endLine", location?.endLine), ("endColumn", location?.endColumn)] {
            if let wanted = expected[key]?.integerValue, wanted != value { mismatch(path + "/" + key, String(wanted), value.map(String.init)) }
        }
    }

    func matches(_ expected: ConformanceData, _ actual: RuntimeCollector.Limit) -> Bool {
        (expected["name"] == nil || expected["name"]?.stringValue == actual.name) && (expected["limit"] == nil || expected["limit"]?.integerValue == actual.limit) && (expected["detailContains"]?.stringValue.map { actual.detail.contains($0) } ?? true)
    }
    ...
    func compareEvent(_ expected: ConformanceData, _ actual: RuntimeCollector.Event, path: String) throws {
        ...
        if let message = actual.message, let wanted = expected["message"] { try channel([wanted], [message], path: path + "/message") }
        if let value = expected["signatureId"]?.stringValue, value != actual.signature { mismatch(path + "/signatureId", value, actual.signature) }
        if let value = expected["accepted"]?.boolValue { check(path + "/accepted", value, actual.accepted ?? false) }
        if let result = actual.result {
            for (key, value) in [("localAccepted", result.localAccepted), ("outboundAttempted", result.outboundAttempted), ("outboundAccepted", result.outboundAccepted), ("anyAccepted", result.anyAccepted)] {
                if let wanted = expected["result"]?[key]?.boolValue { check(path + "/result/" + key, wanted, value) }
            }
        }
        if let value = actual.diagnostic { checkDiagnostic(expected["diagnostic"] ?? .null, value, path: path + "/diagnostic") }
        if let value = actual.limit, !matches(expected["runtimeLimit"] ?? .null, value) { mismatch(path + "/runtimeLimit", "matching limit", value.name) }
    }

    func check(_ path: String, _ expected: Bool, _ actual: Bool) { if expected != actual { mismatch(path, String(expected), String(actual)) } }

    func mismatch(_ path: String, _ expected: String?, _ actual: String?) { differences.append(.init(path: path, expected: expected, actual: actual)) }
}
...
private final class RuntimeNativeHandler: GameEventScriptNativeMessageHandler {
    ...
    func handle(_ message: GameEventScriptMessage, context: GameEventScriptContext) throws {
        if definition["throw"]?.boolValue == true { throw ConformanceExecutionError.invalidInput("Configured native failure") }
        if let code = definition["faultCode"]?.stringValue {
            throw try GameEventScriptExtensionFault(code: code, message: "Configured native fault", programName: definition["faultContext"]?["programName"]?.stringValue, handlerName: definition["faultContext"]?["handlerName"]?.stringValue)
        }
        ...
        for emit in definition.values("emit") {
            if emit["forwardArguments"]?.boolValue == true {
                let arguments = try (0..<message.arguments.count).map { try GameEventScriptMessageArgument(name: message.arguments.nameAt($0), value: message.arguments[$0]) }
...
final class RuntimeCollector: GameEventScriptRuntimeObserver, GameEventScriptPublishSink {
    ...
    func dispatchStarted(_ message: GameEventScriptMessage, signatureID: String) { trace.append(.init(kind: "dispatchStarted", message: message, signature: signatureID)) }

    func dispatchCompleted(_ message: GameEventScriptMessage, signatureID: String) { trace.append(.init(kind: "dispatchCompleted", message: message, signature: signatureID)) }

...
public enum ConformanceReportWriter {
    ...
    public static func identify(_ documents: [ConformanceDocument]) throws -> ConformanceCorpusIdentity {
        guard !documents.isEmpty, Set(documents.map(\.suiteID)).count == documents.count, Set(documents.map(\.formatVersion)).count == 1 else { throw ConformanceReportError.invalidCorpus }
        ...
        return ConformanceCorpusIdentity(markdownFormatVersion: documents[0].formatVersion, sha256: ConformanceSha256.hex(bytes), documentCount: documents.count, caseCount: cases.count)
    }
    ...
    public static func crossLanguage(_ documents: [ConformanceDocument], report: ConformanceRunReport) throws -> String {
        ...
        let cases = documents.flatMap(\.cases)
        guard report.cases.count == cases.count, Set(report.cases.map { $0.testCase.fullID }).count == cases.count else { throw ConformanceReportError.inconsistentResults }
        ...
        let entries = try cases.sorted(by: { scalarLess($0.fullID, $1.fullID) }).map { testCase -> ConformanceData in
            guard let result = byID[testCase.fullID], result.testCase.kind == testCase.kind, result.testCase.level == testCase.level, result.testCase.requiredCore == testCase.requiredCore, result.testCase.requiredOptional == testCase.requiredOptional
            ...
            return object([
                ...
                ("requires", object([("core", strings(testCase.requiredCore.sorted())), ("optional", strings(testCase.requiredOptional.sorted()))])), ("status", .string(result.status)), ("code", .string(result.code)),
            ])
        }
        return json(
            object([
                ...
                (
                    "corpus",
                    object([
                        ("markdownFormatVersion", .number(String(identity.markdownFormatVersion))), ("sha256", .string(identity.sha256)), ("documentCount", .number(String(identity.documentCount))), ("caseCount", .number(String(identity.caseCount))),
                    ])
                ), ("cases", .array(entries)),
            ])
        )
    ...
    public static func full(_ report: ConformanceRunReport) -> String {
        let entries = report.cases.map { result -> ConformanceData in
            ...
            var fields: [(String, ConformanceData)] = [
                ("id", .string(testCase.fullID)), ("suiteId", .string(suiteID)), ("caseId", .string(testCase.id)), ("title", .string(testCase.title)), ("kind", .string(testCase.kind)), ("level", .string(testCase.level)),
                ("categories", testCase.metadata["categories"] ?? .array([])), ("tags", testCase.metadata["tags"] ?? .array([])), ("status", .string(result.status)), ("code", .string(result.code)),
                ("missingCapabilities", strings(result.missingCapabilities)),
                (
                    "mismatches",
                    .array(
                        result.mismatches.map { mismatch in
                            var entry: [(String, ConformanceData)] = [("path", .string(mismatch.path)), ("code", .string(result.performance == nil ? "conformance.assertion.mismatch" : "conformance.performance.regression"))]
                ...
                ), ("diagnostics", .array([])), ("runtimeLimits", .array([])), ("actualAssembler", .null), ("performance", result.performance.map(performanceData) ?? .null),
            ]
        ...
        return json(
            object([
                ("schemaVersion", .number("1")), ("runner", object([("id", .string("ges-swift-conformance")), ("version", .string("0.1.0"))])), ("implementation", object([("id", .string("swift")), ("version", .string("0.1.0"))])),
                ("capabilities", strings(report.capabilities)), ("performanceProfile", report.performanceProfile.map(ConformanceData.string) ?? .null), ("status", .string(report.status)),
                ("summary", object([("total", .number(String(report.cases.count)))] + ["passed", "failed", "skipped", "error"].map { ($0, .number(String(report.count($0)))) })), ("cases", .array(entries)),
            ])
        )
    }
    ...
    public static func markdown(_ report: ConformanceRunReport) -> String {
        ...
        result += "\(report.count("passed")) passed, \(report.count("failed")) failed, \(report.count("error")) errors, \(report.count("skipped")) skipped.\n\n"
        ...
        for entry in report.cases { result += "| \(entry.testCase.fullID) | \(entry.status) | \(entry.code) | \(entry.missingCapabilities.joined(separator: ", ")) |\n" }
        ...
        if !measured.isEmpty {
            ...
            for entry in measured {
                for metric in entry.performance!.metrics {
                    result += "| \(entry.testCase.fullID) | \(metric.id) | \(GesValue.float(metric.measured).asText) | \(GesValue.float(metric.reference).asText) | \(GesValue.float(metric.allowed).asText) | \(metric.unit) |\n"
                }
            }
        }
        return result
                ...
                object(
                    value.metrics.map { metric in
                        (
                            metric.id,
                            object([
                                ("measured", .string(GesValue.float(metric.measured).asText)), ("reference", .string(GesValue.float(metric.reference).asText)), ("allowed", .string(GesValue.float(metric.allowed).asText)),
    ...
    static func object(_ entries: [(String, ConformanceData)]) -> ConformanceData { .object(entries.map { ConformanceEntry(key: $0.0, value: $0.1) }) }

    static func scalarLess(_ a: String, _ b: String) -> Bool { a.unicodeScalars.lexicographicallyPrecedes(b.unicodeScalars) }
    ...
    private static func encode(_ value: ConformanceData, depth: Int) -> String {
        ...
        case .object(let values): return values.isEmpty ? "{}" : "{\n" + values.map { child + quote($0.key) + ": " + encode($0.value, depth: depth + 1) }.joined(separator: ",\n") + "\n" + indent + "}"
        }
    }

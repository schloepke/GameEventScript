...
extension GesCompiler {
    func validate() throws {
        for module in modules {
            for (name, value) in module.constants {
                if constants[name] != nil {
                    throw error("validate.duplicateConstant", value.location, symbol: name, kind: .globalDefinition)
                }
                constants[name] = value
            }
            for record in module.records {
                if records[record.name] != nil || catalog?.types.contains(where: { $0.name == record.name }) == true {
                    throw error("validate.duplicateType", record.location, symbol: record.name, kind: .type)
                }
            ...
            for definition in module.definitions where !definition.kind.hasSuffix("andler") {
                if definitions[definition.signature] != nil {
                    throw error(
                        definition.kind == "predicate" ? "validate.duplicatePredicate" : "validate.duplicateFunction",
                        definition.location,
                        symbol: definition.name, kind: definition.kind == "predicate" ? .predicate : .function)
                }
                if definitions.values.contains(where: { $0.name == definition.name && $0.kind != definition.kind }) {
                    throw error(
                        "validate.predicateFunctionConflict", definition.location, symbol: definition.name,
        ...
        for module in modules {
            for d in module.definitions {
                if !d.kind.hasSuffix("andler") && d.name.contains("_") {
                    throw error(
                        "validate.invalidIdentifierCase", d.location, symbol: d.name,
                        kind: d.kind == "predicate" ? .predicate : .function)
                }
                var names: Set<String> = []
                for p in d.parameters {
                    if !names.insert(p.name).inserted {
                        throw error(
                            d.kind.hasSuffix("andler")
                                ? "validate.duplicateHandlerParameter" : "validate.duplicateDefinitionParameter",
                            p.location, symbol: d.name,
        ...
        for module in modules {
            for d in module.definitions {
                ...
                if d.kind == "predicate", let expression = d.expression,
                    !["boolean", "nothing"].contains(infer(expression, types))
                {
            ...
            for record in module.records {
                ...
                for field in record.fields {
                    if !names.insert(field.name).inserted {
                        throw error("validate.duplicateVariable", field.location, symbol: field.name, kind: .variable)
                    }
    ...
    func knownType(_ type: String) -> Bool {
        [
            "number", "numeric", "nothing", "boolean", "percentage", "vector", "point", "series", "tag", "text", "list",
            "range", "message", "handler", "map", "dice", "unit", "integer", "fractional",
        ].contains(type)
            || type.hasPrefix("quantity:") || records[type] != nil
            || catalog?.types.contains { $0.name == type } == true
    }
    func validateType(_ type: String, _ location: GameEventScriptSourceLocation) throws {
        if !knownType(type) { throw error("validate.invalidTypeConstructor", location, symbol: type, kind: .type) }
    }
    func validateStatements(
        _ statements: [GesStatement], _ initial: Set<String>, _ ancestors: Set<String>,
        _ initialTypes: [String: String] = [:]
    ) throws {
        var names = initial
        ...
        for s in statements {
            ...
            case .letBinding(let name, let expression):
                ...
                if names.contains(name) {
                    throw error("validate.duplicateVariable", s.location, symbol: name, kind: .variable)
                }
                if ancestors.contains(name) {
                    throw error("validate.shadowedVariable", s.location, symbol: name, kind: .variable)
                }
                names.insert(name)
            ...
            case .loop(let name, let e, _, let body):
                try validateExpression(e, visible: names.union(ancestors), types: types)
                if names.contains(name) || ancestors.contains(name) {
                    throw error("validate.shadowedVariable", s.location, symbol: name, kind: .variable)
                }
    ...
    func validateExpression(_ e: GesExpression, visible: Set<String> = [], types: [String: String] = [:]) throws {
        ...
        func arguments(_ args: [GesArgument], _ name: String = "") throws {
            ...
            for argument in args where argument.label != "_" {
                if argument.label.contains("_") {
                    throw error("validate.invalidIdentifierCase", e.location, symbol: name)
                }
                if !labels.insert(argument.label).inserted {
                    throw error("validate.duplicatePublishArgument", e.location, symbol: name, kind: .message)
                }
            }
        ...
        case .handler(let name, let params):
            var labels: Set<String> = []
            for p in params where p.label != "_" {
                if !labels.insert(p.label).inserted {
                    throw error("validate.duplicateHandlerParameter", e.location, symbol: name, kind: .handler)
                }
            }
        ...
        case .seeded(let a, let b):
            ...
            children = [a, b]
        case .call(let name, let args), .message(let name, let args), .extensionCall(_, let name, let args):
        ...
        case .constructor(let name, let args):
            ...
            if ["vector", "point"].contains(name) {
                ...
                let validLabels =
                    labeled.isEmpty
                    || first >= 0 && first + labeled.count <= 3 && labeled.count == args.count
                        && !(first == 0 && labeled.count == 1)
                        && labeled.enumerated().allSatisfy { $0.element.label == ["x", "y", "z"][first + $0.offset] }
                if args.count > 3 || !validLabels {
                ...
                for arg in args {
                    ...
                    if !labels.contains(arg.label) || positional > labels.filter({ $0 == "_" }).count {
                        throw error("validate.invalidTypeConstructor", e.location, symbol: name, kind: .type)
                    }
                }
            }

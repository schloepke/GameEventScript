...
extension GesCompiler {
    func materialize(_ sourceBindings: [GameEventScriptBinding], _ sourceCode: [Instruction]) -> (
        [GameEventScriptBinding], [Instruction]
    ) {
        ...
        let order: [GameEventScriptBinaryBindKind: Int] = [
            .messageHandler: 0, .messageNameHandler: 0, .record: 1, .predicate: 2, .function: 3, .outboundMessage: 4,
            .extensionCall: 5, .externalType: 6,
        ]
        ...
        let bindings = ordered.map { binding in
            ...
            return GameEventScriptBinding(
                kind: binding.kind, name: resolve(binding.name), argumentNames: args,
                entryAddress: binding.entryAddress, id: binding.id, requiredTags: required, excludedTags: excluded,
        ...
        let instructions = sourceCode.map { instruction in
            ...
            let order =
                instruction.opcode == .emitMessageWithTags || instruction.opcode == .publishMessageWithTags
                ? [0, 2, 1, 3, 4, 5, 6] : Array(0...6)
            for slot in order {

...
{
    public static EventScriptModule Parse(string input, string? sourceName = null)
        => EventScriptParser.Parse(input, sourceName);

    public static IReadOnlyList<EventScriptModule> ParseModules(params string[] input)
        => input.Select(script => EventScriptParser.Parse(script)).ToList();

    public static EventScriptModule Merge(params EventScriptModule[] modules)
        => new(
            "AnonymousModule_MERGED",
            "UnknownSource_MERGED",
            modules.SelectMany(module => module.TypeDefinitions).ToArray(),
            modules.SelectMany(module => module.RuleDefinitions).ToArray(),
            modules.SelectMany(module => module.SelectDefinitions).ToArray(),
            modules.SelectMany(module => module.Handlers).ToArray());

    public static LinkedEventScriptModule Link(params string[] inputs)
    {
        var linker = new EventScriptLinkBuilder();
        foreach (var input in inputs)
        {
            linker.AddScript(input);
        }

...
enum NumberTextProbe {
    ...
    static func run(input: String, output: String) throws {
        ...
        let program = try GameEventScriptBuilder().addScript("on Probe(value, text) { emit Result(formatted: value as :Text, number: text as :Number, percentage: text as :Percentage) }").compile()
        let host = try GameEventScriptHost(seed: 0)
        let capture = Capture()
        _ = try host.subscribe(.init(name: "Result", parameters: ["formatted", "number", "percentage"]), handler: capture)
        ...
        for row in requests {
            ...
            if row.kind == "integer", let integer = Int64(row.payload) {
                ...
                if row.kind == "float" { value = .float(number, unit: unit) } else if row.kind == "percentage" && unit == .none { value = .percentage(number) } else { throw ToolError.invalidFixture("Unknown numeric input kind") }
            } else {
                throw ToolError.invalidFixture("Invalid numeric payload")
            }
            capture.message = nil
            capture.count = 0
            host.receive(try .init(name: "Probe", arguments: [.init(name: "value", value: value), .init(name: "text", value: .text(row.text ?? "nothing"))]))
            guard try host.runToCompletion().state == .completed, capture.count == 1, let result = capture.message else { throw ToolError.invalidFixture("Probe did not produce exactly one result") }
            responses.append(try Response(source: describe(value), text: result.arguments[0].asText, number: describe(result.arguments[1]), percentage: describe(result.arguments[2])))
        }
    ...
    private static func describe(_ value: GesValue) throws -> String {
        ...
        guard value.kind == .float || value.kind == .percentage else { throw ToolError.invalidFixture("Non-numeric result") }

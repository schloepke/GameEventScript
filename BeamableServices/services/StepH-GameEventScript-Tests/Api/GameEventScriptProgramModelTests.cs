...
public sealed class GameEventScriptProgramModelTests
{
    ...
    [TestMethod]
    public void CompileWriteReadRewriteAndLoadPreservesPortableMeaning()
    {
        var compiled = Compile();
        var encoded = GameEventScriptProgramWriter.ToArray(compiled);
        var decoded = GameEventScriptProgramReader.Read(encoded);
        var rewritten = GameEventScriptProgramWriter.ToArray(decoded);
        CollectionAssert.AreEqual(encoded, rewritten);

        long resultValue = 0;
        var host = GameEventScriptHost.CreateBuilder().Build();
        host.Load(decoded);
        host.Subscribe("Done", ["result"], new CaptureHandler(message => resultValue = message.Arguments.GetAsInteger(0)));
        Assert.IsTrue(host.Receive(GameEventScriptMessage.Create(
            "Start",
            [new GameEventScriptMessageArgument("value", GesValue.GesInteger(41))])));
        var result = host.RunToCompletion();

        Assert.AreEqual(GameEventScriptExecutionState.Completed, result.State);
        Assert.AreEqual(42L, resultValue);
    ...
    private sealed class CaptureHandler(Action<GameEventScriptMessage> capture) : IGameEventScriptNativeMessageHandler
    {
        public void Handle(GameEventScriptMessage message, GameEventScriptContext context) => capture(message);
    }
}

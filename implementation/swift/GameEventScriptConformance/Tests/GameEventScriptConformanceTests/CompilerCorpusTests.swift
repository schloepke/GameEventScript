...
final class CompilerCorpusTests: XCTestCase {
    ...
    func testCompilerBuilderOwnershipAndOptions() throws {
        let builder = GameEventScriptBuilder().addScript("on Start() { emit First() }").addScript("on Start() { emit Second(1, 2, 3) }")
    ...
    func testRunnerResourceBoundaries() throws {
        ...
        for (value, code) in [
            (ConformanceResourceResult.notFound, "conformance.resource.unavailable"), (.error("fixture unavailable"), "conformance.resource.unavailable"), (.limitExceeded, "conformance.resource.limitExceeded"),
            (.found([0, 1]), "conformance.resource.limitExceeded"), (.found([0]), "conformance.resource.integrityMismatch"),
        ] {
            let result = ConformanceRunner.runCase(test, environment: .init(resourceResolver: FixedResource(value), maximumResourceBytes: 1))
    ...
    func testSharedCompilerCorpus() throws {
        ...
        for path in paths {
            ...
            for test in document.cases {
                ...
                try test.fullID.write(to: root.appendingPathComponent("artifacts/swift/compiler-active-case.txt"), atomically: true, encoding: .utf8)
                var bytes: [UInt8]?
                if let fixture = test.metadata["binaryFixture"]?["relativePath"]?.stringValue { bytes = Array(try Data(contentsOf: root.appendingPathComponent("conformance/fixtures/" + fixture))) }
                // Performance behavior remains checked, without claiming a Swift measurement profile.
                let result = test.kind == "performance" ? ConformanceCompilerRunner.runCase(test) : ConformanceRunner.runCase(test, environment: .init(resourceResolver: FixedResource(bytes.map(ConformanceResourceResult.found) ?? .notFound)))
                if result.status != "passed", ["bytecodeSnapshot", "programBinary"].contains(test.kind), let program = try? ConformanceCompilerRunner.compile(test).first?.program {
                    ...
                    try GameEventScriptProgramDumper.dump(program).write(to: folder.appendingPathComponent(name + ".actual.gesa"), atomically: true, encoding: .utf8)
                    let expected = test.assembler ?? bytes.flatMap { try? GameEventScriptProgramReader.read($0) }.map { GameEventScriptProgramDumper.dump($0) }
                    try expected?.write(to: folder.appendingPathComponent(name + ".expected.gesa"), atomically: true, encoding: .utf8)
                }
        ...
        try JSONSerialization.data(withJSONObject: results, options: [.prettyPrinted, .sortedKeys]).write(to: root.appendingPathComponent("artifacts/swift/compiler-results.json"))
        print("Swift compiler corpus: \(results.filter { $0["status"] as? String == "passed" }.count)/\(results.count) passed")
    }
}

...
public sealed class GameEventScriptPoint2Value : GameEventScriptValue
{
    public static readonly GameEventScriptPoint2Value Zero = new(0m, 0m, null);

    public static GameEventScriptPoint2Value Create(decimal x, decimal y, GameEventScriptDecimalUnit? unit = null)
        => x == 0m && y == 0m && unit is null ? Zero : new GameEventScriptPoint2Value(x, y, unit);

    private GameEventScriptPoint2Value(decimal x, decimal y, GameEventScriptDecimalUnit? unit)
    {
        X = x;
        Y = y;
        Unit = unit;
    ...
    public override GameEventScriptValueKind Kind => GameEventScriptValueKind.Point2;

...
- `Dst` is the raw `DestinationRegister` register as hex.
...
- **Primary 16-bit operand view:** most instructions use `DestinationRegister`, `X`,
...
- **Primary unsigned index operand view:** `Index` is available for `IndexAccess`
  and stores a statically known 1-based selector in the same bytes as `XRegister`.
...
- `DestinationRegister` is the destination frame register for value-producing instructions.
- `X` and `Y` in the opcode table are primary-word 16-bit fields. Cells use
  semantic aliases where one is established, for example `ConditionRegister`,
...
- Pool-backed opcodes use the documented `StringPool` or `UShortListPool`
  indices directly in `X`, `Y`, `DestinationRegister`, or the payload word.

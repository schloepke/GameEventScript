...
final class PerformanceRunnerTests: XCTestCase {
        ...
        func measure(_ test: ConformanceCase, profile: String) throws -> [ConformanceMeasuredMetric] {
            ...
            var result = test.expectation["performance"]!["profiles"]![profile]!["metrics"]!.objectValue!.map { entry in
                ...
                let limit = min(
                    ...
                    number("toleranceRelative").map { reference * (1 + $0) } ?? .infinity,
                    number("toleranceAbsolute").map { reference + $0 } ?? .infinity)
                return ConformanceMeasuredMetric(
                    id: entry.key,
                    value: mode == "nan"
                        ? .nan
                        : mode == "negative"
                            ? -1 : mode == "regression" ? limit + 1 : mode == "improvement" ? 0 : limit,
    ...
    func testPerformanceContractAndReports() throws {
        ...
        let text = try String(
            contentsOf: root.appendingPathComponent("conformance/suites/performance/core.md"), encoding: .utf8)
        let document = try ConformanceMarkdownParser.parse(text)
        let test = try XCTUnwrap(document.cases.first { $0.kind == "performance" })
        func environment(_ mode: String, profile: String = "csharp-dotnet-release-macos-arm64")
            -> ConformanceEnvironment
        {
            .init(
                capabilities: ConformanceEnvironment.supportedCapabilities + ["performance"],
                performanceProfile: profile, performanceProvider: Provider(mode: mode))
        }
        for mode in ["limit", "improvement"] {
            ...
            let report = ConformanceRunReport(
                capabilities: environment(mode).capabilities, cases: [result],
                performanceProfile: environment(mode).performanceProfile)
            let json = try XCTUnwrap(
                JSONSerialization.jsonObject(with: Data(ConformanceReportWriter.full(report).utf8)) as? [String: Any])
        ...
        for mode in ["nan", "negative", "unit", "missing", "duplicate"] {
            XCTAssertEqual(
                ConformanceRunner.runCase(test, environment: environment(mode)).code,
                "conformance.runner.invalidEnvironment", mode)
        }
        XCTAssertEqual(
            ConformanceRunner.runCase(test, environment: environment("limit", profile: "absent")).code,
            "conformance.runner.missingPerformanceProfile")
        let changed = try ConformanceMarkdownParser.parse(
            text.replacingOccurrences(of: "value: \"29\"", with: "value: \"999\""))
        let wrong = try XCTUnwrap(changed.cases.first { $0.kind == "performance" })
        XCTAssertEqual(
            ConformanceRunner.runCase(wrong, environment: environment("mustNotRun")).code,

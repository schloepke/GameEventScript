...
enum ToolFiles {
    ...
    static func expand(_ arguments: [String]) throws -> [String] {
        ...
        func add(_ path: String) throws {
            if seen.insert(Array(try fullPath(path).utf8)).inserted { paths.append(path) }
        }
        for argument in arguments {
            ...
            guard !directory.contains(where: { $0 == "*" || $0 == "?" }), !pattern.contains("**") else {
                throw ToolError.usage(
                    "Wildcards are supported only in file names; recursive patterns are not supported.")
            }
            let expression =
                "\\A"
                + pattern.map { character in
                    character == "*"
                        ? ".*" : character == "?" ? "." : NSRegularExpression.escapedPattern(for: String(character))
                }.joined() + "\\z"
            let regex = try NSRegularExpression(pattern: expression, options: [.dotMatchesLineSeparators])
            let matches = try FileManager.default.contentsOfDirectory(atPath: directory.isEmpty ? "." : directory)
                .filter { name in
            ...
            for name in matches {
                try add(directory.isEmpty ? name : (directory as NSString).appendingPathComponent(name))
            }
        }
        return paths
    }
    ...
    static func read(_ path: String) throws -> GameEventScriptProgram {
        if isBinary(path) {
            return try GameEventScriptProgramReader.read(Array(Data(contentsOf: URL(fileURLWithPath: path))))
        }

...
extension Tool {
            ...
            do {
                ...
                if command == ":unloadAll" || command == ":reload" {
                    ...
                    if command == ":unloadAll" {
                        ...
                        if !options.quiet { io.line("Reloaded all active programs on a fresh host; initialization completed.", toError: true) }
                    }
                    continue
                }
                if command == ":dump" {
                ...
                if command == ":load" {
                    ...
                    if !options.quiet { io.line("Loaded \(path); initialization completed. Handlers remain active.", toError: true) }
                    continue
                }
                let program = try GameEventScriptBuilder().addScript("on initialization {\n" + line + "\n}\n", sourceName: path).compile()
                ...
                guard handlers.count == 1, program.stringConstants[Int(handlers[0].name)] == "initialization" else {
                    io.line("error cli.interactiveInput: Enter handler-body statements, not additional handler declarations.", toError: true)
                    success = false
                    continue
                }
    ...
    func consoleHelp(_ topic: String) -> Bool {
        ...
        default:
            io.line("error cli.consoleCommand: Unknown help topic '\(topic)'. Use :help, :help load, or :help dump.", toError: true)
            return false
        }
    ...
    static func loadPath(_ text: String) throws -> String {
        ...
        if let quote = path.first, quote == "'" || quote == "\"" {
            guard path.count >= 2, path.last == quote else { throw ToolError.usage("The quoted :load path must end with a matching quote.") }
            path = String(path.dropFirst().dropLast()).replacingOccurrences(of: String(repeating: quote, count: 2), with: String(quote))
        }
        guard !path.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else { throw ToolError.usage("Specify one file after :load. Use :help load for examples.") }

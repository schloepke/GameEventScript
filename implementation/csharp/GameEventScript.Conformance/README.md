...
`GameEventScript.Conformance` implements the authoring side of
...
references. This module’s `tests` directory contains the runner,
parser, report, and test-framework adapter tests. Each other C# module owns its
native tests in its own `tests` directory. Cross-module API and repository
gates live under `../verification`; see the [test guide](../verification/README.md).

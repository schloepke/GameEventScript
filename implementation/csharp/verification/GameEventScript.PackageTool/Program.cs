...
namespace GameEventScript.PackageTool;

internal static class Program
{
    private const string RuntimePackage = "GameEventScript.Runtime";
    ...
    [
        ...
        new("GameEventScript.SyntaxHighlighter", [])
    ];
    ...
    private static void VerifyAssemblyReferences(PackageDefinition package, byte[] assembly)
    {
        ...
        foreach (var handle in metadata.AssemblyReferences)
        {
            ...
            if (name.StartsWith("StepH.GameEventScript", StringComparison.Ordinal))
                throw new InvalidDataException($"Package assembly '{package.Id}' references the former package identity '{name}'.");
            if (name.StartsWith("GameEventScript", StringComparison.Ordinal) && !package.Dependencies.Contains(name, StringComparer.Ordinal))

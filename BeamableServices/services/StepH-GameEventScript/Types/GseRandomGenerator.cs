...
public sealed class GseRandomGenerator
{
    private readonly Random _random;
    private readonly Queue<decimal>? _sequence;

    private GseRandomGenerator(Random random, IEnumerable<decimal>? sequence = null)
    {
        _random = random;
        _sequence = sequence == null ? null : new Queue<decimal>(sequence);
    }

    public static GseRandomGenerator Create() => new(new Random());

    public static GseRandomGenerator FromRandom(Random random) => new(random);

    public static GseRandomGenerator FromSeed(int seed) => new(new Random(seed));

    public static GseRandomGenerator FromSequence(params decimal[] values) => new(new Random(), values);

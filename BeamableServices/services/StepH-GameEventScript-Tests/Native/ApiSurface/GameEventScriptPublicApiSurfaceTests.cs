...
public sealed class GameEventScriptPublicApiSurfaceTests
{
    ...
    private static string FindApprovedSnapshotPath()
    {
        ...
        {
            var candidate = Path.Combine(directory.FullName, "Native", "ApiSurface", "PublicApiSurface.approved.txt");
            if (File.Exists(candidate))
            {
                return candidate;
            }

            candidate = Path.Combine(directory.FullName, "StepH-GameEventScript-Tests", "Native", "ApiSurface", "PublicApiSurface.approved.txt");
            if (File.Exists(candidate))
            {
                return candidate;
            }

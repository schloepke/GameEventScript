...
public sealed class GseRuntimeLimits
{
    public static GseRuntimeLimits Default { get; } = new();
...
internal sealed class GseRuntimeBudget(GseContext context, GseRuntimeLimits limits)
{
    ...
    public GseRuntimeLimits Limits { get; } = limits ?? GseRuntimeLimits.Default;
    ...
    public void ReportLimit(string limitName, string detail, int limit)
        => context.RecordDiagnostic(
            GseDiagnosticEventKind.RuntimeLimitReached,
            limitName,
...
internal static class GseRuntimeLimitUtilities
{
    ...
    public static bool TryGetRangeLength(GseValue value, out long length)
    {
        if (value is GseRangeValue range)
        {

...
internal static class GseClrValueConverter
{
    public static GseValue FromClr(object? value)
    {
        if (value is null)
        {
        ...
        if (value is GseValue eventScriptValue)
        {
            return eventScriptValue;
        }

        if (value is GseDiceValue dice)
        {
            return GseValueFactory.Dice(dice);
        }

        switch (value)
        {
            case string text:
                return GseValueFactory.Text(text);
            case bool boolean:
                return GseValueFactory.Boolean(boolean);
            case byte number:
                return GseValueFactory.Integer(number);
            case sbyte number:
                return GseValueFactory.Integer(number);
            case short number:
                return GseValueFactory.Integer(number);
            ...
            case ulong number when number <= long.MaxValue:
                return GseValueFactory.Integer((long)number);
            case ulong number:
                return GseValueFactory.Decimal((decimal)number);
            case float number:
                return GseDecimalValue.GseDecimal(number);
            case double number:
                return GseDecimalValue.GseDecimal(number);
            case decimal number:
        ...
        {
            return GseValueFactory.Dictionary(dictionaryEntries.ToDictionary(x => x.Key, x => FromClr(x.Value), StringComparer.Ordinal));
        }

        if (value is ISet<GseValue> typedSet)
        {
            return GseValueFactory.Set(typedSet);
        }

        if (value is IEnumerable enumerable and not string)
        {
            var list = new List<GseValue>();
            foreach (var item in enumerable)
            {
                list.Add(FromClr(item));
            }

            return GseValueFactory.List(list);
        }

        return GseValueFactory.Dictionary(ExtractObjectMembers(value));
    }

    public static IReadOnlyList<GseValue> FromClrList(IEnumerable<object?>? values)
        => values?.Select(FromClr).ToArray() ?? Array.Empty<GseValue>();
    ...
    private static IReadOnlyDictionary<string, GseValue> ExtractObjectMembers(object value)
    {

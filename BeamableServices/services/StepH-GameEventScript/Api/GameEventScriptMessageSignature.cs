...
public sealed class GameEventScriptMessageSignature
{
    ...
    /// <param name="parameters">An optional collection of parameter names associated with the message.
    /// Can be null if the message does not require parameters.</param>
    ...
    /// <see cref="GameEventScriptMessageSignature"/> based on the message name and signature ID.
    /// </summary>
    ...
    private GameEventScriptMessageSignature(string name, IEnumerable<string>? parameters)
    {
        ...
        Parameters = parameters is null ? [] : parameters.Select(NormalizeParameterName).ToArray();

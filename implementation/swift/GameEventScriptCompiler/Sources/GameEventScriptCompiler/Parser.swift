...
final class GesParser {
    ...
    func location(_ start: GesToken, _ end: GesToken? = nil) -> GameEventScriptSourceLocation {
        let last = end ?? start
        return .init(
            sourceName: source.name, line: start.line, column: start.column,
            endLine: last.endLine, endColumn: last.endColumn, moduleName: module, sourceID: source.id)
    }
    func failure(_ message: String, _ token: GesToken? = nil, code: String = "parse.syntax")
    ...
    func typeName() throws -> String {
        ...
        let builtins = [
            "Nothing", "Number", "Boolean", "Percentage", "Vector", "Point", "Series", "Tag", "Text", "List", "Range",
    ...
    func parameters(_ allowType: Bool = true) throws -> [GesParameter] {
        ...
        repeat {
            ...
            result.append(
                .init(label: unlabeled ? "_" : name, name: name, type: type, location: location(start, previous)))
        } while match(",")
        try expect(")")
        return result
    }
    func parse() throws -> GesModule {
        ...
        if match("module") {
            ...
            if !module.split(separator: ".").allSatisfy({
                $0.unicodeScalars.allSatisfy { GesLexer.lower($0) || GesLexer.digit($0) }
            }) {
                throw failure("Invalid module name.", previous)
            }
        }
        separators()
        ...
        while current.kind != "eof" {
            ...
            if match("constant") {
                ...
                while current.syntaxText != "}" {
                    ...
                    if unlabeled && computed != nil {
                        throw failure("Computed field cannot be a parameter.", fieldStart)
                    }
                    fields.append(
                        .init(
                            name: field, type: type, label: computed == nil ? (unlabeled ? "_" : field) : nil,
                            minimum: low, maximum: high, computed: computed, location: location(fieldStart, previous)))
                    _ = match(",")
                    separators()
                }
                try expect("}")
                ...
                definitions.append(
                    .init(
                        kind: kind, name: name, parameters: params, expression: body, statements: [],
                        required: [], excluded: [], location: location(begin, previous)))
            } else {
                try expect("on")
                newlines()
                ...
                while true {
                    ...
                    repeat {
                        ...
                        if include {
                            if !required.contains(tag) { required.append(tag) }
        ...
        return .init(
            name: module, definitions: definitions, constants: constants, records: records,
            location: location(start, previous))
    }
    ...
    func body() throws -> [GesStatement] {
        if statementDepth >= 32 {
            throw failure("Statement nesting limit exceeded.", code: "parse.sourceNestingExceeded")
        }
        statementDepth += 1
    ...
    func combined(_ kind: GesExpression.Kind, _ first: GesExpression, _ last: GesExpression? = nil) throws
        -> GesExpression
    {
        ...
        return .init(
            kind,
            .init(
                sourceName: a.sourceName, line: a.line, column: a.column, endLine: b.endLine,
                endColumn: b.endColumn, moduleName: module, sourceID: source.id), depth: depth)
    }
    static let precedence: [String: Int] = [
        "->": 1, "default": 2, "|": 3, "&": 4, "zip": 5, "or": 6, "xor": 7, "and": 8,
        "=": 9, "<>": 9, "in": 10, "not in": 10, "starts": 10, "ends": 10, "has": 10, "as": 11, "is": 11,
        "<": 12, ">": 12, "<=": 12, ">=": 12, "+": 13, "-": 13, "*": 14, "/": 14, "div": 14, "mod": 14, "rem": 14,
        "^": 16,
    ]
    func expression(_ minimum: Int = 0) throws -> GesExpression {
        if expressionDepth >= 32 {
            throw failure("Expression nesting limit exceeded.", code: "parse.sourceNestingExceeded")
        }
        expressionDepth += 1

...
private final class GesProgramDump {
    ...
    init(_ program: GameEventScriptProgram) {
        ...
        for (index, binding) in p.bindings.enumerated() {
            ...
            bindLabels.append(
                unique(
                    prefix(binding.kind) + "_"
                        + (name.isEmpty || name == "_" ? String(binding.id == .max ? index : Int(binding.id)) : name),
                    used: &used))
        ...
        for address in labels.sorted() where codeNames[address] == nil {
            if let owner = named.last(where: { $0 <= address }) {
                codeNames[address] = unique(codeNames[owner]! + "_" + String(address), used: &used, firstSuffix: 1)
            }
        }
    }
    func dump(addresses: Bool) -> String {
        output =
            ...
            + separator + "\n\n"
        output +=
            ".gesb " + String(p.formatVersion) + "\n.module " + quote(p.moduleName) + "\n.program-version "
            + String(p.programVersion) + "\n"
        ...
        for index in p.stringConstants.indices {
            output += aligned(textLabels[index]) + ".text " + quote(p.stringConstants[index]) + "\n"
        }
        endRegion("Text")
        ...
        for (index, values) in p.uint16IndexLists.enumerated() {
            ...
            output +=
                aligned(listLabels[index]) + (role == 2 ? ".texts" : role == 1 ? ".registers" : ".u16") + " ["
                + rendered.joined(separator: ", ") + "]"
            if role == 2 && !values.isEmpty {
                output += " // " + values.map { quote(text($0)) }.joined(separator: ", ")
            }
            output += "\n"
        }
        endRegion("Lists")
        ...
        for (index, binding) in p.bindings.enumerated() {
            output +=
                aligned(bindLabels[index]) + ".bind " + enumName(binding.kind) + " id="
                + (binding.id == .max ? "none" : String(binding.id)) + " name=" + textLabels[Int(binding.name)]
                + " args=" + texts(binding.argumentNames)
        ...
        for (address, i) in p.code.enumerated() {
            ...
            if let mapping = p.sourceMap?.entries.first(where: {
                address >= $0.codeStart && address < UInt64($0.codeStart) + UInt64($0.codeLength)
            }),
                let source = p.sourceMap?.sources.first(where: { $0.sourceID == mapping.sourceID })
            {
                generated = false
                ...
                if previousSource != mapping.sourceID || previousLine != line {
                    ...
                    if let archive = p.sourceArchive?.first(where: { $0.sourceID == mapping.sourceID }) {
                        let lines = normalizeLines(archive.text).split(
                            separator: "\n", omittingEmptySubsequences: false)
                        if line < lines.count { content = String(lines[line]) }
                    }
                    output +=
                        ".source-line " + quote(source.sourceName) + " " + String(line + 1) + " | " + content + "\n"
                }
            } else if !generated {
            ...
            if hasLabel && !inline {
                output += aligned(code(UInt16(address))) + (codeComments[address].map { " // " + $0 } ?? "") + "\n"
            }
            if addresses {
    ...
    func operand(_ i: GameEventScriptBytecodeInstruction, _ part: GesOperand, _ position: Int, _ address: Int) -> String
    {
        if part.isRegister {
            ...
            let symbol = p.debugSymbols?.filter {
                $0.registerID == register && address >= $0.codeStart
                    && address < UInt64($0.codeStart) + UInt64($0.codeLength)
            }.min { $0.codeLength < $1.codeLength }
        ...
        case .outboundMessage, .recordReference, .externalReference:
            return binding(i, part).map { bindLabels[$0] } ?? "Bind_none"
    ...
    func comment(_ i: GameEventScriptBytecodeInstruction, _ part: GesOperand) -> String? {
        ...
        if [.outboundMessage, .recordReference, .externalReference].contains(part), let index = binding(i, part) {
            return "\"" + signature(p.bindings[index], escaped: true, tags: true) + "\""
        }
    ...
    func binding(_ i: GameEventScriptBytecodeInstruction, _ part: GesOperand) -> Int? {
        let kind: GameEventScriptBinaryBindKind =
            ...
            ? .outboundMessage
            : part == .recordReference ? .record : i.opcode == .createExternalType ? .externalType : .extensionCall
    ...
    func signature(_ binding: GameEventScriptBinding, escaped: Bool, tags: Bool) -> String {
        ...
        if binding.kind == .messageNameHandler {
            result += " as message"
        } else {
            result += "(" + binding.argumentNames.map(resolve).joined(separator: ", ") + ")"
        }
        if tags {
            if !binding.requiredTags.isEmpty {
                result += " matching " + binding.requiredTags.map { "#" + resolve($0) }.joined(separator: ", ")
            }
            if !binding.excludedTags.isEmpty {
                result += " without " + binding.excludedTags.map { "#" + resolve($0) }.joined(separator: ", ")
            }
        }
        return result
    }
    ...
    func code(_ address: UInt16) -> String {
        address == .max ? "L_none" : codeNames[Int(address)] ?? "L_" + String(address)
    }
    ...
    func escape(_ text: String) -> String {
        text.unicodeScalars.map { $0 == "\\" ? "\\\\" : $0 == "\"" ? "\\\"" : String($0) }.joined()
    }
    func quote(_ text: String) -> String { "\"" + escape(text) + "\"" }
    func aligned(_ label: String) -> String {
        label + ":" + String(repeating: "\t", count: max(1, (20 - label.utf16.count - 1 + 3) / 4))
    }

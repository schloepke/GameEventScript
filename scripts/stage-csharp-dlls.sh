...
highlighter_output="$repository_root/implementation/csharp/GameEventScript.SyntaxHighlighter/src/bin/Release/netstandard2.1"
...
if [ "$build_mode" = "build" ]; then
    dotnet build implementation/csharp/GameEventScript.SyntaxHighlighter/src/GameEventScript.SyntaxHighlighter.csproj --configuration Release -p:Version="$release_version"
...
rm -rf "$staging_root"
mkdir -p "$staging_root/runtime" "$staging_root/compiler" "$staging_root/unity" "$staging_root/syntaxhighlighter"

for extension in dll pdb xml; do
    cp "$highlighter_output/GameEventScript.SyntaxHighlighter.$extension" "$staging_root/syntaxhighlighter/"

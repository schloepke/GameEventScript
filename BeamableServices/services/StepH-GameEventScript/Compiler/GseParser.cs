...
public sealed class GseParser
{
    ...
    public static GseParsedModule Parse(string script, string? sourceName = null)
    {
        ...
        var rawTokens = new GseLexer(normalizedScript).Tokenize().ToList();
        ...
        var tokens = new List<GseToken>();
        var initialErrors = new List<GseSyntaxError>();
        ...
        return new GseParser(tokens, moduleName, resolvedSourceName, initialErrors)
            .ParseGse();
    }
    ...
    private sealed class GseParseException(string message, int line, int column) : Exception($"{message} (line {line}, col {column})")
    {
        ...
        public GseParseException(string message, GseToken token)
            : this(message, token.Line, token.Column, token.EndLine, token.EndColumn)
        {
        }

        public GseParseException(string message, int line, int column, int endLine, int endColumn)
    ...
    private readonly IReadOnlyList<GseToken> _tokens;
    private readonly string _moduleName;
    private readonly string _sourceName;
    private readonly List<GseSyntaxError> _errors;
    private int _index;

    private GseParser(IReadOnlyList<GseToken> tokens, string moduleName, string sourceName, IReadOnlyList<GseSyntaxError> initialErrors)
    {
        _tokens = tokens;
    ...
    private GseSourceLocation CreateRange(GseToken token)
        => new(_sourceName, token.Line, token.Column, token.EndLine, token.EndColumn, _moduleName);

    private GseSourceLocation CreateRange(GseToken start, GseToken end)
        => new(_sourceName, start.Line, start.Column, end.EndLine, end.EndColumn, _moduleName);

    private GseSourceLocation? MergeRanges(GseNode? first, GseNode? last = null)
    {
        ...
        return endLine is null || endColumn is null
            ? null
            : new GseSourceLocation(_sourceName, start.Line, start.Column, endLine, endColumn, _moduleName);
    }

    private T WithRange<T>(T node, GseToken startToken) where T : GseNode
        => node with { SourceRange = CreateRange(startToken, Previous) };

    private T WithRange<T>(T node, GseToken startToken, GseToken endToken) where T : GseNode
    ...
    private static string? TryResolveModuleName(IReadOnlyList<GseToken> tokens)
    {
        var index = 0;
    ...
    private void ParseOptionalModuleDeclaration()
    {
        ...
        {
            throw new GseParseException($"Expected module name but found {Current.Kind}", Current.Line, Current.Column);
        }
    ...
    private IReadOnlyList<StatementNode> ParseStatementsUntil(GseTokenKind closingKind)
    {
        ...
        while (!Is(closingKind))
        {
            ...
            {
                var token = Current;
                AddParseError(new GseParseException($"Expected '{closingKind}' before end of input", token.Line, token.Column));
                return statements;
            }
    ...
    private ArgumentListNode ParseArgumentListAfterFirstArgument(ArgumentNode firstArgument, GseTokenKind closingKind)
    {
    ...
    private LetStatementNode ParseLetStatement()
    {
        ...
        var token = Current;
        throw new GseParseException($"Expected {Be} but found {token.Kind}", token.Line, token.Column);
    }
    ...
    private ForStatementNode ParseForStatement()
    {
        ...
        if (Match(In))
        {
            ...
            {
                throw new GseParseException("Direct ranges are not allowed after 'in'; use 'for item from ... to ...' or iterate a range variable", Current.Line, Current.Column);
            }
        ...
        {
            var token = Current;
            throw new GseParseException($"Expected {In} or 'from' but found {token.Text}", token.Line, token.Column);
        }
    ...
    private DicePatternNode ParseDicePattern()
    {
        ...
        var token = Current;
        throw new GseParseException($"Expected dice pattern but found {token.Text}", token.Line, token.Column);
    }
        ...
        while (true)
        {
            if (Match(GseTokenKind.Is))
            {
                ...
                if (MatchWord("at"))
                {
                    ...
                    var token = Current;
                    throw new GseParseException($"Expected least or most but found {token.Text}", token.Line, token.Column);
                }
                ...
                if (Match(Or))
                {
                    ...
                    var token = Current;
                    throw new GseParseException($"Expected less, more or greater but found {token.Text}", token.Line, token.Column);
                }
    ...
    private CollectionSelectorNode ParseCollectionSelector()
    {
        ...
        {
            SkipNewLines();
            var countToken = Expect(GseTokenKind.Decimal);
    ...
    private string ParseSortDirection()
    {
        ...
        var token = Current;
        throw new GseParseException($"Expected sort direction but found {token.Text}", token.Line, token.Column);
    }
    ...
    private ExpressionNode ParsePrimaryExpression()
    {
        ...
        if (Match(GseTokenKind.UnitDecimal))
        {
        ...
        var token = Current;
        throw new GseParseException($"Unexpected token '{token.Text}'", token.Line, token.Column);
    }
    ...
    private ExpressionNode ParseCollectionFactoryExpression(string collectionType)
    {
        ...
        var token = Current;
        throw new GseParseException($"Expected :select but found {token.Text}", token.Line, token.Column);
    }
    ...
    private GeneratedCollectionExpressionNode ParseGeneratedCollectionExpression(string collectionType)
    {
        ...
        else if (Match(In))
        {
            ...
            if (Current.Kind == Identifier && string.Equals(Current.Text, "from", StringComparison.Ordinal))
            {
                throw new GseParseException("Direct ranges are not allowed after 'in'; use ':select item from ... to ...' or iterate a range value", Current.Line, Current.Column);
            }
        ...
        {
            var token = Current;
            throw new GseParseException($"Expected 'from' or {In} but found {token.Text}", token.Line, token.Column);
        }
    ...
    {
        var startToken = Previous;
        var diceCountToken = Expect(GseTokenKind.Decimal);
    ...
    private CollectionSelectorNode ParseDropSelector()
    {
        ...
        {
            var token = Current;
            throw new GseParseException($"Expected drop scope but found {token.Text}", token.Line, token.Column);
        }
    ...
    private SequenceLiteralExpressionNode ParseSequenceLiteralExpressionCore(GseToken startToken)
    {
        SkipNewLines();
    ...
    private GseToken ParseDiceSideCountToken()
    {
        if (Match(DiceSeparator))
        {
            return Expect(GseTokenKind.Decimal);
        }
        ...
        var current = Current;
        throw new GseParseException($"Expected dice separator but found {current.Kind}", current.Line, current.Column);
    }
    ...
    private int ParsePositiveInteger(GseToken numberToken, string name)
    {
        if (!decimal.TryParse(numberToken.Text, NumberStyles.Number, CultureInfo.InvariantCulture, out var value) ||
            value <= 0 ||
            value != decimal.Truncate(value))
        {
            throw new GseParseException($"Expected positive integer for {name}", numberToken.Line, numberToken.Column);
        }

        if (value > int.MaxValue)
        {
            throw new GseParseException($"{name} is too large", numberToken.Line, numberToken.Column);
        }

        return (int)value;
    }

    private GseToken Expect(GseTokenKind kind)
    {
        if (Is(kind))
        {
            return Advance();
        ...
        var token = Current;
        throw new GseParseException($"Expected {kind} but found {token.Kind}", token);
    }
    ...
    private string ExpectIdentifier()
    {
        ...
        var token = Current;
        throw new GseParseException($"Expected Identifier but found {token.Kind}", token);
    }

    private bool Match(params GseTokenKind[] kinds)
    {
    ...
    private bool Is(GseTokenKind kind) => Current.Kind == kind;
    ...
    private (string ExtensionName, string FunctionName, GseToken EndToken) ParseExtensionSymbol()
    {
    ...
    private bool IsExtensionUnaryArgumentStart()
        => Current.Kind is Identifier or Message or Tag or GseTokenKind.Decimal or Percentage or UnitDecimal or Text or True or False or LeftBracket or LeftParen or Minus or Has or Empty or Not;
    ...
    private string ExpectArgumentLabel()
    {
        ...
        var token = Current;
        throw new GseParseException($"Expected argument label but found {token.Kind}", token);
    }
    ...
    private string ParseTypeName()
    {
        ...
        {
            var token = Current;
            throw new GseParseException($"Expected type name but found {token.Kind}", token);
        }
    ...
    private void RequireStatementSeparatorOrClosing(GseTokenKind closingKind)
    {
        if (Is(closingKind))
        {
            return;
    ...
    private GseToken Current => _tokens[_index];

    private GseToken Previous => _tokens[_index - 1];

    private void AddParseError(GseParseException exception)
    {
    ...
    private bool SynchronizeStatement(GseTokenKind closingKind)
    {
        while (!Is(EndOfFile))
        {
            if (Is(closingKind))

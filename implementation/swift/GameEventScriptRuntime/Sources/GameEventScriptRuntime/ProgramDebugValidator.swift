...
extension GameEventScriptProgramValidator {
    static func validateDebug(_ p: GameEventScriptProgram) throws {
        for (index, symbol) in (p.debugSymbols ?? []).enumerated() {
            if (symbol.name != "_" && !GesNames.identifier(symbol.name)) || symbol.codeLength == 0
                || UInt64(symbol.codeStart) + UInt64(symbol.codeLength) > UInt64(p.code.count)
                || symbol.registerID == .max
            {
        ...
        if let map = p.sourceMap {
            for (index, source) in map.sources.enumerated() {
                if source.sourceID != index || source.sourceName.isEmpty || source.sha256.count != 32
                    || source.lineStartByteOffsets.first != 0
                {
            ...
            for (index, entry) in map.entries.enumerated() {
                ...
                if entry.codeLength == 0 || entry.sourceByteLength == 0 || UInt64(entry.codeStart) < end
                    || UInt64(entry.codeStart) + UInt64(entry.codeLength) > UInt64(p.code.count)
                    || UInt64(entry.sourceStartByteOffset) + UInt64(entry.sourceByteLength)
                        > UInt64(source.sourceByteLength)
                {
                    throw failure(.invalidSourceMap, 33, index)
                }
        ...
        if let archive = p.sourceArchive {
            ...
            for (index, source) in archive.enumerated() {
                if !ids.insert(source.sourceID).inserted || source.sourceName.isEmpty {
                    throw failure(.invalidSourceArchive, 34, index)
                }
        ...
        if let map = p.sourceMap, let archive = p.sourceArchive {
            ...
            for (index, metadata) in map.sources.enumerated() {
                guard let source = byID[metadata.sourceID], GesText.scalarEqual(source.sourceName, metadata.sourceName),
                    source.utf8Content.count == metadata.sourceByteLength,
                    GesSha256.digest(source.utf8Content) == metadata.sha256
                else { throw failure(.sourceMetadataMismatch, 33, index) }
                let bytes = source.utf8Content
                func boundary(_ offset: UInt32) -> Bool {
                    offset == 0 || offset == bytes.count
                        || Int(offset) < bytes.count && bytes[Int(offset)] & 0xc0 != 0x80
                }
                ...
                for (mappingIndex, entry) in map.entries.enumerated() where entry.sourceID == metadata.sourceID {
                    if !boundary(entry.sourceStartByteOffset)
                        || !boundary(entry.sourceStartByteOffset + entry.sourceByteLength)
                    {
                        throw failure(.invalidSourceMap, 33, mappingIndex)
                    }
                }
            }
        }
        if let build = p.buildMetadata, build.compilerID.isEmpty || build.compilerVersion.isEmpty {
            throw failure(.invalidProgram, 48)
        }
    }
    ...
    static func validateOpaque(_ p: GameEventScriptProgram) throws {
        ...
        for (index, section) in p.opaqueSections.enumerated() {
            ...
            if type == 0 || type == .max || section.originalOrdinal < 0 || section.flags & 1 != 0 {
                throw failure(.unknownRequiredSection, type, index)
            }
            ...
            if type == 32 && p.debugSymbols != nil || type == 33 && p.sourceMap != nil
                || type == 34 && p.sourceArchive != nil || type == 48 && p.buildMetadata != nil
            {
                throw failure(.duplicateSection, type, index)
            }
            if [32, 33, 34, 48].contains(type) {
                if section.sectionVersion == 1 && section.flags & 0xf0 == 0 {
                    throw failure(.invalidProgram, type, index)
                }

...
enum GesCallbacks {
    static func extensionCall(_ i: GameEventScriptBytecodeInstruction, _ s: GesVmState, _ c: GameEventScriptContext)
        throws
    {
        let destination = Int(i.word0)
        let registers = s.list(i.word2)
        guard let binding = s.linked!.extensionBindings[i.word1], let function = s.linked!.extensions[i.word1],
            binding.argumentNames.count == registers.count
        else {
        ...
        defer {
            ...
            if s.error == nil {
                if fault == .boundaryUnderflow {
                    s.fail("runtime.randomStackUnderflow")
                } else if fault == .unbalanced {
                    s.fail("runtime.randomScopeImbalance")
                }
            }
            call.end()
        }
    ...
    static func constructor(_ i: GameEventScriptBytecodeInstruction, _ s: GesVmState, _ c: GameEventScriptContext)
        throws
    {
        let destination = Int(i.word0)
        guard let binding = s.linked!.constructorBindings[i.word1], let constructor = s.linked!.constructors[i.word1]
        else {
            s.set(destination, .nothing)
        ...
        guard labels.count == s.stageLength, labels.count == binding.argumentNames.count,
            labels.count == constructor.definition.parameters.count, !labels.contains("_")
        else {
            s.set(destination, .nothing)
            return
        }
        ...
        defer { call.end() }
        do { try constructor.invoke(call) } catch let fault as GameEventScriptExtensionFault { throw fault } catch let
            fault as GesRuntimeError
        { throw fault } catch {
            s.set(destination, .nothing)

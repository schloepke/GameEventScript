...
JSON conformance therefore compares finite binary64 results by ULP distance.
The current default is 4096 ULPs, chosen to retain the older decimal-rounded
fixtures and allow small cross-library differences. A test or individual step
...
The JSON described here is the test interchange, not the future product wire
message format.

- Int64 and binary64 payloads are strings so JSON implementations cannot first
  round them through an implementation-defined JSON number type.
...
- `NaN` is accepted in expected JSON as the spelling of invalid mathematics and

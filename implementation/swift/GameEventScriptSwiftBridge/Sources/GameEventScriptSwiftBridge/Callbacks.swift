...
public struct GameEventScriptSwiftMessageHandler: GameEventScriptNativeMessageHandler {
    ...
    public func handle(_ message: GameEventScriptMessage, context: GameEventScriptContext) throws {
        try body(message, context)
    }
}
...
extension GameEventScriptHost {
    ...
    public func subscribe(
        ...
        priority: Int = 0, handler: @escaping (GameEventScriptMessage, GameEventScriptContext) throws -> Void
    ) throws -> GameEventScriptSubscription {
        try subscribe(
            signature, handler: GameEventScriptSwiftMessageHandler(handler),
            matchingTags: matchingTags, withoutTags: withoutTags, priority: priority)
    }
    ...
    public func subscribeMessageName(
        _ name: String, matchingTags: [String] = [], withoutTags: [String] = [], priority: Int = 0,
        handler: @escaping (GameEventScriptMessage, GameEventScriptContext) throws -> Void
    ) throws -> GameEventScriptSubscription {
        try subscribeMessageName(
            name, handler: GameEventScriptSwiftMessageHandler(handler),
            matchingTags: matchingTags, withoutTags: withoutTags, priority: priority)
    }
}
...
extension GameEventScriptMessage {
    ...
    public init(
        name: String, swiftArguments: [(String?, any GameEventScriptSwiftValueConvertible)], tags: [String] = []
    ) throws {
        try self.init(
            name: name,
            arguments: swiftArguments.map {
                try GameEventScriptMessageArgument(name: $0.0, value: $0.1.toGesValue())
            }, tags: tags)
    }
}
extension GameEventScriptMessageSignature {
    ...
    public func withSwiftArguments(_ values: [String: any GameEventScriptSwiftValueConvertible]) throws
        -> GameEventScriptMessage
    {
        ...
        for (key, value) in values {
            ...
            guard normalized.updateValue(value, forKey: name) == nil else {
...
extension GameEventScriptContext {
    ...
    @discardableResult public func emit(
        _ name: String, swiftArguments: [(String?, any GameEventScriptSwiftValueConvertible)], tags: [String] = []
    ) throws -> Bool { emit(try .init(name: name, swiftArguments: swiftArguments, tags: tags)) }
    ...
    @discardableResult public func publish(
        _ name: String, swiftArguments: [(String?, any GameEventScriptSwiftValueConvertible)], tags: [String] = []
    ) throws -> GameEventScriptPublishResult {

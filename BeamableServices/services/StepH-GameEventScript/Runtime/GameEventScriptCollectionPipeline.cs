...
internal sealed class GameEventScriptCollectionPipeline
{
    private GameEventScriptCollectionPipeline(ExpressionNode sourceExpression, IReadOnlyList<CollectionSelectorNode> prefixSelectors, CollectionSelectorNode terminalSelector)
    {
    ...
    public static bool TryCreate(CollectionAccessExpressionNode expression, out GameEventScriptCollectionPipeline pipeline)
    {
        ...
        pipeline = new GameEventScriptCollectionPipeline(source, prefixSelectors, terminalSelector);
        return true;
    }

    public IEnumerable<GameEventScriptValue> ApplyPrefix(IEnumerable<GameEventScriptValue> items, GameEventScriptProjectionEvaluator projection)
    {
    ...
    public static IEnumerable<GameEventScriptValue> ApplyComposableSelector(
        IEnumerable<GameEventScriptValue> items,
        CollectionSelectorNode selector,
        GameEventScriptProjectionEvaluator projection)
    {
    ...
    private static IEnumerable<GameEventScriptValue> Filter(
        IEnumerable<GameEventScriptValue> items,
        GameEventScriptProjectionEvaluator projection,
        string identifier,
    ...
    private static IEnumerable<GameEventScriptValue> Select(
        IEnumerable<GameEventScriptValue> items,
        GameEventScriptProjectionEvaluator projection,
        string identifier,

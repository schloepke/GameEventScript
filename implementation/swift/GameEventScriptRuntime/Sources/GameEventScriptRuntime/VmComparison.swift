...
enum GesComparison {
    ...
    static func equal(_ a: GesValue, _ b: GesValue) -> Bool {
        ...
        if let av = a.spatialValue, let bv = b.spatialValue { return a.kind == b.kind && a.unit == b.unit && near(av.x, bv.x) && near(av.y, bv.y) && near(av.z, bv.z) }
        if a.kind == .dice && b.kind == .dice || a.kind == .dice && b.kind == .integer || b.kind == .dice && a.kind == .integer { return !a.hasUnit && !b.hasUnit && a.asInteger == b.asInteger }
        if a.kind == .dice && b.isNumeric || b.kind == .dice && a.isNumeric { return !a.hasUnit && !b.hasUnit && near(a.asNumber, b.asNumber) }
        if let av = a.floatRangeValue, let bv = b.floatRangeValue { return near(av.from, bv.from) && near(av.to, bv.to) && near(av.step, bv.step) }
    ...
    static func order(_ a: GesValue, _ b: GesValue) -> Int? {
        ...
        if let av = a.textValue, let bv = b.textValue { return GesText.scalarEqual(av, bv) ? 0 : GesText.scalarLess(av, bv) ? -1 : 1 }
    ...
    static func extreme(_ a: GesValue, _ b: GesValue, maximum: Bool) -> GesValue {
        ...
        if (a.numericOnly || a.kind == .boolean) && (b.numericOnly || b.kind == .boolean) {
            ...
            if let av = a.integerValue, let bv = b.integerValue { comparison = av < bv ? -1 : av > bv ? 1 : 0 } else { comparison = a.asNumber < b.asNumber ? -1 : a.asNumber > b.asNumber ? 1 : 0 }

...
func run() throws -> Int32 {
    ...
    guard let enumerator = FileManager.default.enumerator(at: corpusURL, includingPropertiesForKeys: [.isRegularFileKey]) else { throw ToolError.invalidArguments("Cannot enumerate corpus") }
    let paths = enumerator.compactMap { $0 as? URL }.filter { $0.pathExtension == "md" }.sorted { $0.path < $1.path }
    let documents = try paths.map { path in do { return try ConformanceMarkdownParser.parse(Array(Data(contentsOf: path))) } catch { throw ToolError.invalidArguments("\(path.path): \(error)") } }
    let resolver = try binaryFixtures.map { try FileResources(directory: URL(fileURLWithPath: $0), documents: documents) }
    ...
    if calibratePerformance {
        guard !measurePerformance, let provider else { throw ToolError.invalidArguments("Choose calibration or regression verification") }
        var count = 0
        for document in documents {
            for test in document.cases where test.kind == "performance" {
                let correctness = ConformanceRunner.runCase(test, environment: .init(capabilities: ConformanceEnvironment.supportedCapabilities))
                ...
                let result = ConformanceRuntimeRunner.runCase(test, programs: [.init(id: test.sources[0].programID, program: try workload.compile())])
                guard correctness.status == "skipped", result.status == "passed" else { throw ToolError.invalidFixture("Performance correctness failed: " + test.fullID) }
    ...
    let environment = ConformanceEnvironment(
        ...
        performanceProfile: provider.map { _ in SwiftPerformanceProvider.profile },
    ...
    for (name, text) in [("ConformanceResults.json", ConformanceReportWriter.full(report)), ("ConformanceResults.md", ConformanceReportWriter.markdown(report)), ("SwiftResults.json", compact)] {
        try Data(text.utf8).write(to: destination.appendingPathComponent(name), options: .atomic)
    }
    print("Swift: \(report.count("passed")) passed, \(report.count("failed")) failed, \(report.count("error")) errors, \(report.count("skipped")) skipped; \(documents.count) documents.")
    var runtimePassed = true
    if let runtimePrograms {
        guard let binaryFixtures else { throw ToolError.invalidArguments("--runtime-programs requires --binary-fixtures") }
        let compactObject = try JSONSerialization.jsonObject(with: Data(compact.utf8)) as! [String: Any]
        runtimePassed = try verifyRuntime(documents, directory: URL(fileURLWithPath: runtimePrograms), binaryFixtures: URL(fileURLWithPath: binaryFixtures), destination: destination, corpus: compactObject["corpus"]!)
    }
    let failing = report.cases.filter { $0.status == "failed" || ($0.status == "error" && !(allowIncomplete && $0.code == "conformance.runner.missingCoreCapability")) }
...
struct FileResources: ConformanceResourceResolver {
    ...
    init(directory: URL, documents: [ConformanceDocument]) throws {
        ...
        for test in documents.flatMap(\.cases) {
            guard let fixture = test.metadata["binaryFixture"], let id = fixture["resourceId"]?.stringValue, let relative = fixture["relativePath"]?.stringValue else { continue }
            let path = root.appendingPathComponent(relative).standardizedFileURL.resolvingSymlinksInPath()
            guard path.path.hasPrefix(root.path + "/"), paths[id] == nil || paths[id] == path else { throw ToolError.invalidFixture("Invalid or conflicting resource path for \(id)") }
            paths[id] = path
        }
        self.paths = paths
    }
...
func verifyFixtures(_ directory: URL) throws {
    ...
    for line in manifest.split(separator: "\n").dropFirst() {
        ...
        guard ConformanceSha256.hex(bytes) == fields[4] else { throw ToolError.invalidFixture("Hash mismatch: \(fields[1])") }
        do {
            let document = try ConformanceMarkdownParser.parse(bytes)
            guard fields[2] == "valid", document.suiteID == fields[5], document.cases.map(\.id) == [fields[6]] else { throw ToolError.invalidFixture("Unexpected parse success/identity: \(fields[1])") }
        } catch let error as ConformanceParseError { guard fields[2] == "invalid", error.diagnostic.code == fields[7] else { throw ToolError.invalidFixture("Unexpected diagnostic: \(fields[1]): \(error)") } }
        count += 1
    }

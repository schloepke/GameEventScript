...
| Deterministic Dice result literals, descending duplicate-preserving order, source and parsed text roundtrips, nested values, exact parse bounds, and unchanged random consumption | all `runtime.dice-literals` cases, including `source-values`, `text-roundtrips`, `parse-valid`, `parse-invalid`, `depth-*`, `items-*`, `nested-items-*`, and `source-rejects-*` |
| Constructor-shaped Vector/Point text, positional and labeled numeric components, exact binary64/unit roundtrips, nested data, fallback, and shared depth/item bounds | all `runtime.spatial-literals` cases: `vector-*` and `point-*` |
...
| Binary resources cover executable call depth, live caller frames, staging, record creation/casts, and each individual handler; control-flow joins require balanced frame/stage lengths | `program.binary-format/valid-direct-call`, `program.binary-format/valid-nested-call`, `program.binary-format/underdeclared-depth-direct-call`, `program.binary-format/underdeclared-depth-nested-call`, `program.binary-format/underdeclared-registers-direct-call`, `program.binary-format/underdeclared-registers-nested-call`, and all 13 `program.binary-format/*-resource-*` cases |
| Exactly reaching the processed-message limit while draining all work emits no limit observation, for native/script handlers and completion/frame pumps | the eight `runtime.host-observer/processed-limit-{script,native}-{completion,frame}-{1,2}` cases; below-limit control: `runtime.host-observer/processed-limit-below-script-completion`; stop/resume controls: `runtime.host-observer/emit-and-runtime-limit-order` and the three `runtime.host-observer/processed-limit-pending-*` cases |

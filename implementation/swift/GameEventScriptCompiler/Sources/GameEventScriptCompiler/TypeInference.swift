...
extension GesCompiler {
    func infer(_ expression: GesExpression, _ types: [String: String], _ visiting: Set<String> = []) -> String {
        ...
        case .binary(let op, let a, let b):
            if ["=", "<>", "<", ">", "<=", ">=", "in", "not in", "inValues", "startsWith", "endsWith"].contains(op) { return "boolean" }
            if ["and", "or", "xor", "->", "default"].contains(op) {
                let left = child(a)
                let right = child(b)
                return predicate(left) && predicate(right) ? left == "nothing" && right == "nothing" && op == "default" ? "nothing" : "boolean" : "unknown"
            }
            return "other"
    ...
    func validateSeed(_ expression: GesExpression, _ types: [String: String]) throws {
        ...
        func explicit(_ e: GesExpression) -> Bool {
            ...
            case .literal(let value):
                return !value.hasUnit && value.kind == .float && value.asNumber.isFinite && value.asNumber.rounded(.towardZero) == value.asNumber && value.asNumber >= -9_223_372_036_854_775_808 && value.asNumber < 9_223_372_036_854_775_808

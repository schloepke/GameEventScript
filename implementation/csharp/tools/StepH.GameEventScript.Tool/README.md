...
`StepH.GameEventScript.Tool` is a separate .NET console application packaged as
a .NET tool with command name `ges`. It targets .NET 8 and references the
portable Core and C# bridge. Terminal and filesystem integration belong to this application
or the C# adapters; the Core does not depend on the tool.
...
```sh
dotnet run --project implementation/csharp/tools/StepH.GameEventScript.Tool --configuration Release -- --help
```
...
```sh
dotnet pack implementation/csharp/tools/StepH.GameEventScript.Tool --configuration Release
dotnet tool install StepH.GameEventScript.Tool --version 0.1.0 --add-source ./artifacts/csharp/tool/packages --tool-path ./artifacts/csharp/tool/install

...
public sealed class GameEventScriptPublicApiSurfaceTests
{
    ...
    public void CompilerTypesStayInternal()
    {
        var leakedTypes = typeof(GameEventScriptBuilder)
            ...
            .GetExportedTypes()
            .Where(type =>
                IsNamespace(type, "GameEventScript.Compiler"))
    ...
    public void PublicTypesAndDependenciesFollowAssemblyBoundaries()
    {
        ...
        Assert.AreEqual("GameEventScript.Runtime", runtime.GetName().Name);
        Assert.AreEqual("GameEventScript.Compiler", compiler.GetName().Name);
        Assert.AreEqual("GameEventScript.CSharpBridge", bridge.GetName().Name);
        Assert.AreEqual("GameEventScript.Conformance", conformance.GetName().Name);
        ...
        Assert.HasCount(0, runtime.GetTypes().Where(type => IsNamespace(type, "GameEventScript.Compiler")));
        Assert.HasCount(0, runtime.GetExportedTypes().Where(type => IsNamespace(type, "GameEventScript.CSharpBridge") || IsNamespace(type, "GameEventScript.Conformance")));
        ...
        AssertProductReferences(conformance, "GameEventScript.Runtime", "GameEventScript.Compiler");
    }
    ...
    public void PublicApiDocumentationAndPragmasRemainComplete()
    {
        ...
        var pragmaDirectives = Directory.EnumerateFiles(TestRepositoryPaths.CSharpDirectory, "*.cs", SearchOption.AllDirectories)
    ...
    {
        return Path.Combine(TestRepositoryPaths.RepositoryTestDirectory, "ApiSurface", "PublicApiSurface.approved.txt");
    }
    ...
    private static void AssertProductReferences(Assembly assembly, params string[] expected)
    {
        ...
        Assert.IsFalse(references.Any(name => name.StartsWith("StepH.GameEventScript", StringComparison.Ordinal)), "Product assemblies must not depend on the former package identity.");
        CollectionAssert.AreEquivalent(expected, references.Where(name => name.StartsWith("GameEventScript", StringComparison.Ordinal)).ToArray(), assembly.GetName().Name);

                ...
                while (!IsCompleted && vmState.State == Processing && !session.RuntimeBudget.IsExhausted && opcodesExecuted < maxSteps)
                {
                    ...
                    switch (instruction.OpCode)
                    {
                        ...
                        case StreamMin:
                        {
                            ...
                            GesVmRegisterStreamTerminals.VmStreamMin(ref dst, ref vmState.Register(instruction.XSlot), instruction.YSlot, instruction.AU, this);
                        ...
                        {
                            var dst = vmState.CreateNothing();
                            GesVmRegisterStreamTerminals.VmStreamMax(ref dst, ref vmState.Register(instruction.XSlot), instruction.YSlot, instruction.AU, this);
                            vmState.Register(instruction.DestinationSlot) = dst;
                            break;
                        }
                        ...
                        {
                            var dst = vmState.CreateNothing();
                            GesVmRegisterStreamTerminals.VmStreamOneWeighted(ref dst, ref vmState.Register(instruction.XSlot), instruction.AU, instruction.BU, instruction.CU, this, vmState.RandomGenerator);
                            vmState.Register(instruction.DestinationSlot) = dst;
                            break;
                        }
                        ...
                        {
                            var dst = vmState.CreateNothing();
                            GesVmRegisterStreamTerminals.VmStreamTakeWeighted(ref dst, ref vmState.Register(instruction.XSlot), instruction.ImmediateY, instruction.AU, instruction.BU, instruction.CU, this, vmState.RandomGenerator);
                            vmState.Register(instruction.DestinationSlot) = dst;
                            break;
                        }

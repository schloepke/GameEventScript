...
public sealed class GameEventScriptDocumentationConsistencyTests
{
    ...
    {
        var publicApi = File.ReadAllText(Path.Combine(FindLibraryProjectDirectory(), "Documentation", "Specification", "PublicApi.md"));
    ...
    public void NormativeDocumentationIsIndexedAndAllLocalLinksResolve()
    {
        ...
        var documentationDirectory = Path.Combine(projectDirectory, "Documentation");
        var specificationDirectory = Path.Combine(documentationDirectory, "Specification");
        ...
        var indexedNormativeFiles = ExtractLocalLinkTargets(index)
            .Where(target => target.StartsWith("Specification/", StringComparison.Ordinal))
            .Select(RemoveFragment)
            .OrderBy(path => path, StringComparer.Ordinal)
            .ToArray();

        CollectionAssert.AreEqual(normativeFiles, indexedNormativeFiles, "Documentation/README.md must link every normative specification exactly once.");
        ...
        var testsDirectory = Path.Combine(Path.GetDirectoryName(projectDirectory)!, "StepH-GameEventScript-Tests");
        ...
        var retentionInventory = File.ReadAllText(Path.Combine(projectDirectory, "NativeTestRetention.md"));
    ...
    {
        var specificationDirectory = Path.Combine(FindLibraryProjectDirectory(), "Documentation", "Specification");
    ...
    public void LexerWordVocabularyAppearsInNormativeGrammar()
    {
        ...
        var language = File.ReadAllText(Path.Combine(projectDirectory, "Documentation", "Specification", "Language.md"));

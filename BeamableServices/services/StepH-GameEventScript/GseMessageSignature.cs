...
public sealed class GseMessageSignature
{
    public const string UnlabeledParameterName = "_";

    public static readonly GseMessageSignature Empty = new(string.Empty, []);

    public static GseMessageSignature MessageSignature(string name, IEnumerable<string>? parameters) => new(name, parameters);

    public GseMessage CreateMessage(params (string name, GseValue value)[] arguments) => GseMessage.Message(Name, arguments);
    ...
    public GseMessageSignature(string name, IEnumerable<string>? parameters)
    {
    ...
    public bool Matches(GseMessage message) => string.Equals(Name, message.Name, StringComparison.Ordinal) && string.Equals(SignatureId, message.SignatureId, StringComparison.Ordinal);

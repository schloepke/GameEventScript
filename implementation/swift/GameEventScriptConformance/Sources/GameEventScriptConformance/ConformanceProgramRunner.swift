...
public enum ConformanceProgramRunner {
    ...
    public static func runBinaryCase(
        _ testCase: ConformanceCase, bytes: [UInt8], comparisonProgram: GameEventScriptProgram? = nil
    ) -> ConformanceCaseResult {
        do {
            guard testCase.kind == "programBinary" else {
                throw ConformanceExecutionError.invalidInput("Expected a binary case")
            }
            let fixture = try testCase.metadata.required("binaryFixture")
            let expected = try testCase.expectation.required("binary")
            guard fixture["sha256"]?.stringValue == ConformanceSha256.hex(bytes) else {
                throw ConformanceExecutionError.invalidInput("Fixture SHA-256 mismatch")
            }
            var differences: [ConformanceMismatch] = []
            func check(_ data: ConformanceData, _ key: String, _ value: String?, _ path: String = "/binary") {
                ...
                if wantedString != value {
                    differences.append(.init(path: path + "/" + key, expected: wantedString, actual: value))
                }
            }
            let program: GameEventScriptProgram
            do { program = try GameEventScriptProgramReader.read(bytes) } catch let error
                as GameEventScriptProgramFormatError
            {
                let name = String(describing: error.code)
                check(
                    expected, "outcome",
                    error.code.rawValue <= GameEventScriptProgramFormatErrorCode.tooManyEntries.rawValue
            ...
            if fixture["compareCompiledRuntime"]?.boolValue == true {
                guard let comparisonProgram else {
                    throw ConformanceExecutionError.invalidInput("Missing independently compiled comparison Program")
                }
    ...
    private static func result(_ test: ConformanceCase, _ differences: [ConformanceMismatch], error: String? = nil)
        -> ConformanceCaseResult
    {
        .init(
            testCase: test, status: error != nil ? "error" : differences.isEmpty ? "passed" : "failed",
            code: error != nil
                ? "conformance.runner.invalidEnvironment"
                : differences.isEmpty ? "conformance.passed" : "conformance.assertion.mismatch",
            missingCapabilities: [], mismatches: differences, technicalDetails: error)
    }
}

...
struct RunOptions {
    ...
    init(_ args: [String]) throws {
        ...
        while index < args.count {
            ...
            if argument == "--args" {
                guard index + 1 < args.count, !Self.isOption(args[index + 1]) else { throw ToolError.usage("Specify at least one value after --args.") }
                group = true
                suppliedArguments = true
                continue
            }
            ...
            if ["--scenario", "--arg", "--seed", "--max-messages", "--max-steps"].contains(argument) {
                ...
                if argument == "--scenario" {
                    ...
                    guard let parsed = Int64(value), Self.integerSpelling(value, signed: true) else { throw ToolError.usage("The seed must be a signed 64-bit integer.") }
                    seed = parsed
                } else {
                    guard let parsed = Int32(value), parsed > 0, Self.integerSpelling(value, signed: false) else { throw ToolError.usage("\(argument) requires a positive 32-bit integer.") }
                    if argument == "--max-messages" { limits.maxProcessedEventsPerRun = Int(parsed) } else { limits.maxExecutionSteps = Int(parsed) }
                }
                ...
                guard !argument.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else { throw ToolError.usage("Program paths must not be empty.") }
    ...
    static func isOption(_ text: String) -> Bool {
        ...
        return bytes.count > 1 && bytes[0] == 45 && !(48...57).contains(bytes[1]) && !(bytes.count > 2 && bytes[1] == 46 && (48...57).contains(bytes[2]))
    }
}
...
extension Tool {
    func runPrograms(_ arguments: [String]) throws -> Int {
        ...
        guard binaries == 0 || binaries == paths.count else { throw ToolError.usage("Run accepts either sources compiled together or binaries loaded separately; inputs cannot be mixed.") }
        ...
        if main && !programs.contains(where: Self.hasMain) {
            io.line("error cli.missingMain: No handler matches Main(args). Add 'on Main(args)', use --scenario, or use --interactive.", toError: true)
            return 1
        }
        guard try session.pump() else { return 1 }
        if main {
            let message = try GameEventScriptMessage(name: "Main", arguments: [.init(name: "args", value: .list(options.arguments))])
    ...
    static func hasMain(_ program: GameEventScriptProgram) -> Bool {
        program.bindings.contains {
            ...
            return $0.kind == .messageNameHandler || ($0.kind == .messageHandler && $0.argumentNames.count == 1 && program.stringConstants[Int($0.argumentNames[0])] == "args")
        }
    }
}

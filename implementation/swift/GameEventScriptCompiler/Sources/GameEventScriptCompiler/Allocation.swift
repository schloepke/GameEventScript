...
extension GameEventScriptBytecodeInstruction {
    ...
    func replacing(_ fields: [UInt16]) -> Self {
        .init(opcode: opcode, unitAndFlags: unitAndFlags, word0: fields[0], word1: fields[1], word2: fields[2], payload: UInt64(fields[3]) | UInt64(fields[4]) << 16 | UInt64(fields[5]) << 32 | UInt64(fields[6]) << 48)
    }
}
...
extension GesCompiler {
    func allocate(_ routine: GesRoutine) throws {
        ...
        for (index, instruction) in routine.code.enumerated() {
            ...
            for slot in GameEventScriptCompilerSupport.listSlots(instruction, text: false) { registers += lists[Int(fields[slot])].map(Int.init) }
        ...
        for (index, instruction) in routine.code.enumerated() {
            ...
            case .jump, .jumpIfTrue, .jumpIfFalse, .jumpIfNotTrue, .jumpIfNothing:
                ...
                if start <= index { for register in 0..<routine.registers { if let interval = intervals[register], interval.0 < start, interval.1 >= start, interval.1 < index { intervals[register] = (interval.0, index) } } }
            default: break
            }
        }
        var mapping: [Int: Int] = [:]
        ...
        let temporaries = intervals.filter { !routine.pinned.contains($0.key) }.sorted { a, b in a.value.0 == b.value.0 ? a.key < b.key : a.value.0 < b.value.0 }
        ...
        for index in routine.code.indices {
            ...
            for slot in GameEventScriptCompilerSupport.registerSlots(instruction) { fields[slot] = UInt16(mapping[Int(fields[slot])]!) }
            for slot in GameEventScriptCompilerSupport.listSlots(instruction, text: false) { fields[slot] = UInt16(list(lists[Int(fields[slot])].map { mapping[Int($0)]! })) }

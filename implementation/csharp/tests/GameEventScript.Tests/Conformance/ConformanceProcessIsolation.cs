...
using GameEventScript.Conformance.Worker;
...
internal static class ConformanceProcessIsolation
{
    ...
    private static async Task<ProcessCapture> ExecuteAsync(IReadOnlyList<string> arguments, string directory, int timeoutMilliseconds)
    {
        ...
        var worker = Path.Combine(TestRepositoryPaths.Root, "artifacts", "csharp", "conformance-worker", "bin", configuration, "net8.0", "GameEventScript.Conformance.Worker.dll");

...
final class ProgramBinaryBootstrapTests: XCTestCase {
    ...
    func testSharedBinaryFixtureReadValidateRewrite() throws {
        ...
        for testCase in document.cases {
            guard let fixture = testCase.metadata["binaryFixture"],
                let relativePath = fixture["relativePath"]?.stringValue
            else { continue }
            let bytes = Array(
                try Data(contentsOf: repository.appendingPathComponent("conformance/fixtures/" + relativePath)))
            ...
            do {
                ...
                if let name = expected["moduleName"]?.stringValue {
                    XCTAssertEqual(program.moduleName, name, testCase.fullID)
                }
                for (key, actual) in [
                    ("requiredRegisterCount", Int(program.requiredRegisterCount)),
                    ("requiredCallStackDepth", Int(program.requiredCallStackDepth)),
                    ("opaqueSectionCount", program.opaqueSections.count),
                ] {
                    if let value = expected[key]?.numberValue {
                        XCTAssertEqual(String(actual), value, testCase.fullID + " " + key)
                    }
                }
                ...
                if let exact = expected["rewriteByteExact"]?.boolValue {
                    XCTAssertEqual(rewrite == bytes, exact, testCase.fullID)
                }
                if let hash = expected["rewriteSha256"]?.stringValue {
                    XCTAssertEqual(ConformanceSha256.hex(rewrite), hash, testCase.fullID)
                }
                XCTAssertEqual(
                    try GameEventScriptProgramWriter.bytes(GameEventScriptProgramReader.read(rewrite)), rewrite,
                    testCase.fullID)
                ...
                XCTAssertEqual(
                    name.prefix(1).uppercased() + name.dropFirst(), expected["errorCode"]?.stringValue, testCase.fullID)
                for (key, actual) in [
                    ("byteOffset", error.byteOffset), ("sectionType", error.sectionType.map(Int.init)),
                    ("entryIndex", error.entryIndex),
                ] {
                    if let value = expected[key]?.numberValue {
                        XCTAssertEqual(actual.map(String.init), value, testCase.fullID + " " + key)
                    }
                }
            }
            count += 1

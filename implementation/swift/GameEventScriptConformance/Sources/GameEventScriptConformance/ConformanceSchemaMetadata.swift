...
extension ConformanceSchema {
    static func validateMetadata(_ node: Node, limits: ConformanceParserLimits) throws {
        if let random = node["random"] {
            ...
            guard random.entries!.count == 1 else { throw fail("invalidValue", "random requires exactly one configuration.", random) }
            ...
            if let entropy = random["entropy"] {
                ...
                guard !text.isEmpty, text.utf8.count % 2 == 0, text.utf8.allSatisfy({ (48...57).contains($0) || (65...70).contains($0) }) else { throw fail("invalidValue", "Entropy must contain uppercase hexadecimal byte pairs.", entropy) }
            }
        }
        ...
        if let count = node["hostCount"] {
            ...
            guard value > 0, limits.maxHostsPerTest > 0, value <= UInt64(limits.maxHostsPerTest) else { throw fail("invalidValue", "hostCount exceeds parser limits.", count) }
        }
        ...
        if let performance = node["performance"] {
            ...
            guard try unsigned(required(performance, "iterations"), max: UInt64(UInt32.max)) > 0 else { throw fail("invalidValue", "Performance iterations must be positive.", performance) }
    ...
    static func validateNativeHandlers(_ node: Node) throws {
        ...
        for (index, item) in try array(node).enumerated() {
            try closed(item, ["id", "message", "parameters", "messageName", "priority", "initiallySubscribed", "throw", "faultCode", "faultContext", "actions", "emit"])
            let id = try item["id"].map(identifier) ?? nativeID(index)
            guard ids.insert(id).inserted else { throw fail("duplicateId", "Duplicate native handler ID '\(id)'.", item) }
            _ = try string(required(item, "message"))
            if let parameters = item["parameters"] {
                let values = try stringList(parameters)
                if item["messageName"]?.boolean == true && !values.isEmpty { throw fail("invalidValue", "Name-only handlers cannot constrain parameters.", item) }
            }
            try boolFields(item, ["messageName", "initiallySubscribed", "throw"])
            if let priority = item["priority"] { _ = try integer(priority, min: Int64(Int32.min), max: Int64(Int32.max)) }
            if let code = item["faultCode"] {
                let text = try string(code)
                guard !text.unicodeScalars.allSatisfy(\.properties.isWhitespace), !text.hasPrefix("runtime."), item["throw"]?.boolean != true else {
                    throw fail("invalidValue", "faultCode must be a nonblank caller code outside runtime. without throw: true.", code)
                }
            }
            if let context = item["faultContext"] {
                guard item["faultCode"] != nil else { throw fail("invalidValue", "faultContext requires faultCode.", context) }
                try closed(context, ["programName", "handlerName"])
                for field in ["programName", "handlerName"] { if let value = context[field], value.scalar != .null { _ = try string(value) } }
            }
            ...
            if let emits = item["emit"] {
                for emit in try array(emits) {
                    ...
                    guard (emit["forwardArguments"]?.boolean == true) != (emit["args"] != nil) else { throw fail("invalidValue", "Emit requires forwardArguments: true or args.", emit) }
    ...
    static func validateActions(_ node: Node) throws {
        ...
        for item in try array(node) {
            ...
            if let error = item["expectError"] {
                ...
                guard supplied[0] == "loadProgram", item["expectResult"] == nil, error["phase"]?.string == "link" else { throw fail("invalidValue", "Only loadProgram accepts a link expectError, without expectResult.", error) }
            }
        }
    }
    ...
    static func validateHostReferences(_ node: Node, _ syntax: ConformanceMarkdownCase) throws {
        ...
        if let descriptors = node["sources"]?.items { for source in descriptors { programs.insert(source["program"]?.string ?? "main") } } else if syntax.sources.count == 1 { programs.insert("main") }
        let deferred = try node["deferredPrograms"].map { try stringList($0, ids: true) } ?? []
        guard deferred.count == Set(deferred).count else { throw fail("duplicateId", "Duplicate deferred program ID.", node) }
        guard Set(deferred).isSubset(of: programs) else { throw fail("unknownReference", "Unknown deferred program ID.", node) }
    ...
    static func validateBinaryFixture(_ node: Node) throws {
        try closed(node, ["id", "resourceId", "relativePath", "sha256", "compilerId", "compilerVersion", "programVersion", "compareCompiledRuntime", "derivation"])
        ...
        guard !path.isEmpty, !path.contains("\\"), !path.contains(":"), components.allSatisfy({ !$0.isEmpty && $0 != "." && $0 != ".." }) else {
        ...
        guard !(try string(required(node, "compilerVersion"))).isEmpty else { throw fail("invalidValue", "Compiler versions cannot be empty.", node) }
    ...
    static func sha256(_ node: Node) throws {
        ...
        guard text.utf8.count == 64, text.utf8.allSatisfy({ (48...57).contains($0) || (65...70).contains($0) }) else { throw fail("invalidValue", "SHA-256 requires 64 uppercase hexadecimal digits.", node) }
    }

    static func validateBinaryExpectation(_ node: Node) throws {
        let successFields = ["rewriteByteExact", "rewriteSha256", "moduleName", "requiredRegisterCount", "requiredCallStackDepth", "opaqueSectionCount"]
        ...
        guard valid == (node["errorCode"] == nil) else { throw fail("invalidValue", "Binary errorCode is required exactly for error outcomes.", node) }
        if !valid && successFields.contains(where: { node[$0] != nil }) { throw fail("invalidValue", "Binary error outcomes cannot constrain success fields.", node) }
        if let code = node["errorCode"] {
            let names = [
                "InvalidMagic", "UnsupportedFormatVersion", "InvalidHeaderFlags", "InvalidHeaderSize", "FileSizeMismatch", "FileTooLarge", "TooManySections", "TruncatedSectionHeader", "TruncatedSectionPayload", "SectionTooLarge",
                "InvalidSectionType", "InvalidSectionFlags", "InvalidSectionReserved", "UnsupportedSectionVersion", "UnsupportedCompression", "MissingRequiredSection", "DuplicateSection", "UnknownRequiredSection", "InvalidPayloadLength",
                "InvalidUtf8", "TooManyEntries", "InvalidStringIndex", "InvalidListIndex", "InvalidBindingKind", "DuplicateBindingId", "InvalidEntryAddress", "InvalidOpcode", "InvalidOperand", "InvalidJumpAddress", "InvalidCallAddress",
                "CyclicCallGraph", "InvalidResourceMetadata", "InvalidDebugSymbol", "InvalidSourceMap", "InvalidSourceArchive", "SourceMetadataMismatch", "ReaderLimitExceeded", "InvalidProgram",
            ]
            guard names.contains(try string(code)) else { throw fail("invalidValue", "Unknown binary format error code.", code) }
        }
    ...
    static func validateCompileMetadata(_ node: Node) throws {
        ...
        if let resources = node["programResources"] {
            ...
            guard !resources.entries!.isEmpty else { throw fail("missingField", "Program resources require a constraint.", resources) }
        ...
        if let handlers = node["handlerResources"] {
            for item in try array(handlers) {
                ...
                guard item.entries!.count >= 2 else { throw fail("missingField", "Handler resources require a constraint.", item) }
            }
        }
    }
    ...
    static func validatePerformance(_ node: Node) throws {
        ...
        for profile in profiles {
            ...
            guard !metrics.isEmpty else { throw fail("invalidCardinality", "Performance profiles require metrics.", profile.value) }
            for metric in metrics {
                ...
                for field in ["reference", "maximum", "toleranceRelative", "toleranceAbsolute"] {
                    if let number = item[field] {
                        guard let text = number.number ?? number.string, let value = Double(text), value.isFinite, value >= 0 else { throw fail("invalidValue", "Performance values must be finite and nonnegative.", number) }
                    }
                }
                guard ["maximum", "toleranceRelative", "toleranceAbsolute"].contains(where: { item[$0] != nil }) else { throw fail("missingField", "Performance requires a maximum or tolerance.", item) }
                let unit = try string(required(item, "unit"))
                let base = unit.hasSuffix("/iteration") ? String(unit.dropLast(10)) : unit
                guard ["ns", "us", "ms", "s", "B", "KiB", "count"].contains(base) else { throw fail("invalidValue", "Unknown performance unit.", item) }
            }
        }
    }
}

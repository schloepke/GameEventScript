# Game Event Script Conformance Runner V1

This document defines the normative execution and result contract for the
normalized test model in `ConformanceMarkdownV1.md`. The authoring parser and
...
`ConformanceMarkdownV1.md`. If a required core capability is absent, the case is
...
`PortableNumberSemantics.md` and the declared non-negative maximum. The mode
...
`PortableDiagnostics.md`; `technicalDetails` may be emitted only as explicitly

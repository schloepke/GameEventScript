...
public sealed class GameEventScriptPublicApiSurfaceTests
{
    ...
    {
        var assembly = typeof(GameEventScriptCompiled).Assembly;
    ...
    private static string FindApprovedSnapshotPath()
    {
        ...
        {
            var candidate = Path.Combine(directory.FullName, "Api", "PublicApiSurface.approved.txt");
            if (File.Exists(candidate))
            {
                return candidate;
            }

            candidate = Path.Combine(directory.FullName, "StepH-GameEventScript-Tests", "Api", "PublicApiSurface.approved.txt");
            if (File.Exists(candidate))
            {
                return candidate;
            }

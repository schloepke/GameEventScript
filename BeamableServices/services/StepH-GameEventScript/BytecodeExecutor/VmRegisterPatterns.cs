...
internal static class VmRegisterPatterns
{
    internal static void VmHasPattern(ref this VmValue dst, ref VmValue source, GameEventScriptBytecodePatternKind pattern, short count, ushort faceEntryAddress, IVmStreamEntryEvaluator evaluator, ushort destinationSlot)
    {
    ...
    internal static void VmTakePattern(ref this VmValue dst, ref VmValue source, GameEventScriptBytecodePatternKind pattern, short count, ushort faceEntryAddress, IVmStreamEntryEvaluator evaluator, ushort destinationSlot)
    {
        if (pattern == CountFace)
        {
            var face = dst.OwningState.CreateNothing();
    ...
    private static void HasPatternDice(ref VmValue dst, int[] dice, GameEventScriptBytecodePatternKind pattern, short count, ushort faceEntryAddress, IVmStreamEntryEvaluator evaluator)
    {
        switch (pattern)
        {
            case CountAny:
    ...
    private static void HasPatternDiceFace(ref VmValue dst, int[] dice, ref VmValue face, short count)
    {
        var faceNumber = face.AsNumeric;
        if (!double.IsFinite(faceNumber))
        {
    ...
    private static void HasPatternList(ref VmValue dst, VmListObject list, GameEventScriptBytecodePatternKind pattern, short count, ushort faceEntryAddress, IVmStreamEntryEvaluator evaluator)
    {
        switch (pattern)
        {
            case CountAny:
    ...
    private static void HasPatternListFace(ref VmValue dst, VmListObject list, ref VmValue face, short count)
    {
        var matches = 0;
        for (var i = 0; i < list.Length; i++) if (list.Items[i].EqualsValue(ref face)) matches++;
        dst.SetBoolean(matches >= count);
    ...
    private static void HasPatternBuffer(ref VmValue dst, VmValue[] items, int length, GameEventScriptBytecodePatternKind pattern, short count, ushort faceEntryAddress, IVmStreamEntryEvaluator evaluator)
    {
    ...
    private static void HasPatternBufferFace(ref VmValue dst, VmValue[] items, int length, ref VmValue face, short count)
    {
        var matches = 0;
    ...
    private static void TakePatternDice(ref VmValue dst, int[] dice, GameEventScriptBytecodePatternKind pattern, short count, ushort faceEntryAddress, IVmStreamEntryEvaluator evaluator)
    {
    ...
    private static void TakePatternList(ref VmValue dst, VmListObject list, GameEventScriptBytecodePatternKind pattern, short count, ushort faceEntryAddress, IVmStreamEntryEvaluator evaluator)
    {
        TakePatternBuffer(ref dst, list.Items, list.Length, pattern, count, faceEntryAddress, evaluator, diceResult: false);
    }

    private static void TakePatternBuffer(ref VmValue dst, VmValue[] items, int length, GameEventScriptBytecodePatternKind pattern, short count, ushort faceEntryAddress, IVmStreamEntryEvaluator evaluator, bool diceResult)
    {
        switch (pattern)
        {
            case CountFace:
    ...
    private static void SetTakenByFace(ref VmValue dst, VmValue[] items, int length, ref VmValue face, int count, bool diceResult)
    {
        if (diceResult)
        {
    ...
    private static void TakeFullHouse(ref VmValue dst, VmValue[] items, int length, bool diceResult)
    {
        for (var i = 0; i < length; i++)
        {
            var tripleCount = 1;
    ...
    private static void TakeStraight(ref VmValue dst, VmValue[] items, int length, bool diceResult)
    {
    ...
    private static bool IsFullHouseList(VmListObject list)
    {
    ...
    private static bool IsStraightList(VmListObject list) => IsStraightItems(list.Items, list.Length);

    private static bool IsStraightItems(VmValue[] items, int length)
    {
    ...
    private static bool ReadStream(ref VmValue dst, IVmStream stream, out VmValue[] items, out int length)
    {
        var buffer = Array.Empty<VmValue>();
        length = 0;

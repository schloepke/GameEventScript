...
final class ConformanceCorpusTests: XCTestCase {
    ...
    func testSharedCorpusIdentityCapabilitiesAndAPIs() throws {
        ...
        XCTAssertEqual(documents.count, 96)
        XCTAssertEqual(documents.flatMap(\.cases).count, 1679)
        ...
        let reference = try XCTUnwrap(JSONSerialization.jsonObject(with: Data(contentsOf: repository.appendingPathComponent("conformance/cross-language/CSharpReferenceResults.json"))) as? [String: Any])
        ...
        for (left, right) in zip(actualCases, referenceCases) {
            ...
            XCTAssertEqual(left["requires"] as? NSDictionary, right["requires"] as? NSDictionary, left["id"] as? String ?? "")
        }
        ...
        for result in report.cases {
            ...
            if missing.isEmpty {
                XCTAssertEqual(result.status, "passed", "\(result.testCase.fullID): \(result.mismatches) \(result.technicalDetails ?? "")")
            } else {
    ...
    func testSharedMarkdownBootstrapFixtures() throws {
        ...
        for line in manifest.split(separator: "\n").dropFirst() {
            ...
            if fields[2] == "valid" {
                ...
                XCTAssertThrowsError(try ConformanceMarkdownParser.parse(bytes)) { error in XCTAssertEqual((error as? ConformanceParseError)?.diagnostic.code, fields[7], fields[1]) }
            }
        }
    }
    ...
    func testHashAndCanonicalReportDeterminism() throws {
        ...
        XCTAssertEqual(ConformanceSha256.hex(Array("abc".utf8)), "BA7816BF8F01CFEA414140DE5DAE2223B00361A396177A9CB410FF61F20015AD")
        XCTAssertEqual(ConformanceSha256.hex(Array("abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq".utf8)), "248D6A61D20638B8E5C026930C3E6039A33CE45964FF2167F6ECEDD419DB06C1")
        let documents = try documents()
        let report = ConformanceRunner.runCorpus(documents, environment: apiEnvironment)
        let reverse = Array(documents.reversed())
        XCTAssertEqual(try ConformanceReportWriter.crossLanguage(documents, report: report), try ConformanceReportWriter.crossLanguage(reverse, report: ConformanceRunner.runCorpus(reverse, environment: apiEnvironment)))
    ...
    func testRunnerDetectsMismatchesAndMissingCapabilities() throws {
        ...
        let changed = try ConformanceMarkdownParser.parse(Array(text.replacingOccurrences(of: "matches: false", with: "matches: true").utf8))
        ...
        let forged = ConformanceRunner.runDocument(valid, environment: .init(capabilities: ["performance", "message-api"]))
    ...
    func testClosedSchemasAndCanonicalTransportNumbers() throws {
        ...
        for number in ["1.0", "1E+20", "-0", "1e020", "0.00001"] {
            XCTAssertThrowsError(try ConformanceMarkdownParser.parse(document(number))) { XCTAssertEqual(($0 as? ConformanceParseError)?.diagnostic.code, "conformance.schema.invalidValue", number) }
        }
        for number in ["0", "0.0001", "1e-5", "10000000000000000", "1e17", "NaN", "Infinity", "-Infinity"] { XCTAssertNoThrow(try ConformanceMarkdownParser.parse(document(number)), number) }
        for text in [document("1", extraCase: "  surprise: true"), document("1", extraExpectation: "  surprise: true")] {
            XCTAssertThrowsError(try ConformanceMarkdownParser.parse(text)) { XCTAssertEqual(($0 as? ConformanceParseError)?.diagnostic.code, "conformance.schema.unknownField") }
        }
    ...
    func testOriginalByteRangesPreserveBOMNewlinesAndScalars() throws {
        let path = repository.appendingPathComponent("conformance/fixtures/MarkdownV1/Valid/minimal-bytecode-snapshot.md")
        let original = try String(contentsOf: path, encoding: .utf8).replacingOccurrences(of: "emit Done()", with: "emit Done(\"😀\")")
        for newline in ["\n", "\r\n", "\r"] {
            ...
            func extract(_ range: ConformanceSourceRange) -> String { String(decoding: content[range.byteOffset..<(range.byteOffset + range.byteLength)], as: UTF8.self) }

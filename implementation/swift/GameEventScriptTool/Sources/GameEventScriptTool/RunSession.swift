...
final class RunSession {
    ...
    func summary() {
        io.line(
            "Run completed: \(messages) messages, \(opcodes) opcodes, \(emits) emits, \(publishes) publishes.",
            toError: true)
    }
}
...
final class RunObserver: GameEventScriptRuntimeObserver {
    ...
    private func trace(_ text: String) {
        if options.verbose {
            io.line(options.color && options.interactive ? Highlighting.paint(text, 33) : text, toError: true)
        }
    }
        ...
        if !message.arguments.isEmpty {
            text +=
                ...
                + (0..<message.arguments.count).map {
                    message.arguments.nameAt($0) + ": "
                        + (message.arguments[$0].kind == .text
                            ? quoted(message.arguments[$0].asText) : message.arguments[$0].description)
                }.joined(separator: ", ") + ")"
        }
    ...
    func console(_ message: GameEventScriptMessage, error: Bool) {
        ...
        for index in 0..<message.arguments.count {
            ...
            if options.color && !error && value.kind != .text {
                text +=
                    [.integer, .float, .percentage].contains(value.kind)
                    ? Highlighting.paint(value.description, 34) : highlighting.render(value.description)
            } else {
    ...
    private struct ConsoleHandler: GameEventScriptNativeMessageHandler {
        ...
        func handle(_ message: GameEventScriptMessage, context: GameEventScriptContext) throws {
            observer.console(message, error: error)
        }
    }
    private struct ErrorCodeHandler: GameEventScriptNativeMessageHandler {
        let observer: RunObserver
        func handle(_ message: GameEventScriptMessage, context: GameEventScriptContext) throws {
            guard message.arguments.count == 1, ["code", "_"].contains(message.arguments.nameAt(0)) else {
                throw try invalid()
            }
            ...
            guard [.integer, .float].contains(value.kind), value.unit == .none,
                number.isFinite, (0...255).contains(number), number == number.rounded(.towardZero)
            else { throw try invalid() }
            observer.exitCode = Int(number)
        }
        private func invalid() throws -> GameEventScriptExtensionFault {
            try .init(
                code: "cli.errorCodeArgument",
                message: "ErrorCode requires one code argument: a unitless integer from 0 to 255, or nothing.")
        }
    }
}
